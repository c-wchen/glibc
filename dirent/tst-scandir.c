...
static void verify_empty(const char *dirname)
{
...
static void make_file(const char *dirname, const char *filename)
{
    char *name = NULL;
...
static void freelist(struct D(dirent) **list, size_t n)
{
...
static int select_a(const struct D(dirent) *d)
{
    return d->d_name[0] == 'a';
}
