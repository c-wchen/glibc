...
static void check(const char *funcName)
{
    sigset_t set;
...
static void signalmask(int how, int signum)
{
    sigset_t set;
    sigemptyset(&set);
...
static void signalpending(int signum, const char *msg)
{
    sigset_t set;
    sigemptyset(&set);
    if (sigpending(&set) != 0) {
...
static void handler(int __attribute__((unused)) signum)
{
    handlerCalled ++;
}
