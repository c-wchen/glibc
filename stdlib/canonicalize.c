...
static bool file_accessible(char const *file)
{
...
static bool _GL_ATTRIBUTE_PURE suffix_requires_dir_check(char const *end)
{
    ...
    while (ISSLASH(*end)) {
        ...
        switch (*end++) {
            default:
                return false;  /* An ordinary file name component is OK.  */
            case '\0':
                return true; /* Trailing "/" is trouble.  */
            case '.':
                break;        /* Possibly "." or "..".  */
        }
...
static bool dir_check(char *dir, char *dirend)
{
    strcpy(dirend, dir_suffix);
    return file_accessible(dir);
}

static idx_t get_path_max(void)
{
# ifdef PATH_MAX
...
static char *realpath_stk(const char *name, char *resolved, struct realpath_bufs *bufs)
{
    char *dest;
    char const *start;
    char const *end;
...
char *
attribute_compat_text_section __old_realpath(const char *name, char *resolved)
{
    if (resolved == NULL) {
        __set_errno(EINVAL);
        return NULL;
