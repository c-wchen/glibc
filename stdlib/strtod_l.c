...
/* Set errno and return an overflowing value with sign specified by
   NEGATIVE.  */
static FLOAT overflow_value(int negative)
{
    __set_errno(ERANGE);
...
/* Set errno and return an underflowing value with sign specified by
   NEGATIVE.  */
static FLOAT underflow_value(int negative)
{
    __set_errno(ERANGE);
...
static FLOAT round_and_return(mp_limb_t *retval, intmax_t exponent, int negative,
...
FLOAT ____STRTOF_INTERNAL(const STRING_TYPE *nptr, STRING_TYPE **endptr, int group,
                          locale_t loc)
{
