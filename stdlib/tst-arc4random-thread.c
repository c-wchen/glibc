...
static void generate_arc4random(unsigned char *bytes, size_t size)
{
    int i;
...
static void generate_arc4random_buf(unsigned char *bytes, size_t size)
{
    arc4random_buf(bytes, size);
}

static void generate_arc4random_uniform(unsigned char *bytes, size_t size)
{
...
/* Sort blob elements by length first, then by comparing the data
   member.  */
static int compare_blob(const void *left1, const void *right1)
{
...
static void copy_result_to_global(struct dynarray_blob *result)
{
...
static void *inner_thread(void *closure)
{
...
static void *get_one_blob_thread(void *closure)
{
...
static void fork_thread_subprocess(void *closure)
{
    struct subprocess_args *arg = closure;
...
/* Continuously fork subprocesses to obtain a little bit of
   randomness.  */
static void *fork_thread(void *closure)
{
    void (*func)(unsigned char *, size_t) = closure;
...
static void run_outer_threads(void (*func)(unsigned char *, size_t))
{
...
static int do_test_func(const char *fname, void (*func)(unsigned char *, size_t))
{
