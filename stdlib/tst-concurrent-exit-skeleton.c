...
static void exit_with_flush(int code)
{
    fflush(stdout);
...
static void fn_final(void)
{
    TEST_COMPARE_STRING(crumbs, expected);
    exit_with_flush(0);
}

_Noreturn static void child(void)
{
    enum { nthreads = 8 };
...
static int do_test(void)
{
    ...
    TEST_VERIFY_EXIT(ATEXIT(fn1) == 0);
    --slots_remaining;
    TEST_VERIFY_EXIT(ATEXIT(fn3) == 0);
    --slots_remaining;
    TEST_VERIFY_EXIT(ATEXIT(fn1) == 0);
    --slots_remaining;
