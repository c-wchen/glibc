...
static int string_to_fp(mpfr_t f, const char *s, mpfr_rnd_t rnd)
{
    mpfr_clear_overflow();
...
void print_fp(FILE *fout, mpfr_t f, const char *suffix)
{
    if (mpfr_inf_p(f)) {
...
static const char *suffix_to_print(bool overflow, bool underflow, bool underflow_before_rounding,
                                   bool with_comma)
{
    if (overflow) {
...
static void round_str(FILE *fout, const char *s, int prec, int emin, int emax,
                      bool ibm_ld)
{
    mpfr_t max_value;
    mpfr_t f;
...
static void round_for_all(FILE *fout, const char *s)
{
