...
static void generate_arc4random(unsigned char *bytes)
{
...
static void generate_arc4random_buf(unsigned char *bytes)
{
    arc4random_buf(bytes, random_size);
}

static void generate_arc4random_uniform(unsigned char *bytes)
{
...
static void subprocess(int run, int process_index, void (*func)(unsigned char *))
{
...
static int index_compare(const void *left1, const void *right1)
{
...
static int do_test_func(void (*func)(unsigned char *bytes))
{
    /* Collect random data.  */
    ...
    for (int run = 0; run < runs; ++run)
        for (int process_index = 0; process_index < processes; ++process_index)
            ...
            = (struct index) {
            .run = run, .process_index = process_index
        };
