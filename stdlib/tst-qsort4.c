...
static int cmp(const void *a1, const void *b1, void *closure)
{
...
static void check_one_sort(signed char *array, int length)
{
...
static void check_combinations(int length, signed char *start, int offset)
{
    if (offset == length) {
...
static int do_test(void)
{
    ...
    check_one_sort((signed char[20]) {
        5, 12, 16, 10, 14, 11, 9, 13, 8, 15,
        0, 17, 3, 7, 1, 18, 2, 19, 4, 6
    }, 20);


    /* A permutation that appeared during adversarial testing for the
       quicksort pass.  */
    check_one_sort((signed char[16]) {
        15, 3, 4, 2, 1, 0, 8, 7, 6, 5, 14,
        13, 12, 11, 10, 9
    }, 16);
