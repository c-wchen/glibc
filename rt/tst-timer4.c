...
static void sig1_handler(int sig, siginfo_t *info, void *ctx)
{
    int err = 0;
    if (sig != SIGRTMIN) {
        err |= 1 << 0;
...
static void sig2_handler(int sig, siginfo_t *info, void *ctx)
{
    int err = 0;
    if (sig != SIGRTMIN + 1) {
        err |= 1 << 0;
    }
    if (info->si_signo != SIGRTMIN + 1) {
        err |= 1 << 1;
    }
    if (info->si_code != SI_TIMER) {
        err |= 1 << 2;
...
static int check_ts(const char *name, const struct timespec *start,
