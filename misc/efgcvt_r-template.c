...
int __FCVT_R(FLOAT_TYPE value, int ndigit, int *decpt, int *sign,
             char *buf, size_t len)
{
    ssize_t n;
    ssize_t i;
...
int __ECVT_R(FLOAT_TYPE value, int ndigit, int *decpt, int *sign,
             char *buf, size_t len)
{
    int exponent = 0;
    ...
    } else if (__FCVT_R(value, MIN(ndigit, NDIGIT_MAX) - 1,
                        decpt, sign, buf, len)) {
        return -1;
    }
