...
static void send_vsyslog(int options)
{
...
static void send_syslog(int options)
{
    for (size_t i = 0; i < array_length(facilities); i++) {
        for (size_t j = 0; j < array_length(priorities); j++) {
            int facility = facilities[i];
...
static bool check_syslog_message(const struct msg_t *msg, int msgnum, int options,
                                 pid_t pid)
{
...
static void send_syslog_large(int options)
{
...
static void send_vsyslog_large(int options)
{
    int facility = LOG_USER;
    int priority = LOG_INFO;
...
static bool check_syslog_message_large(const struct msg_t *msg, int msgnum, int options,
                                       pid_t pid)
{
...
static void send_openlog(int options)
{
...
static void send_openlog_large(int options)
{
    /* Define a non-default IDENT and a not default facility.  */
    openlog(OPENLOG_IDENT, options, LOG_LOCAL0);
...
static bool check_openlog_message(const struct msg_t *msg, int msgnum,
...
static bool check_openlog_message_large(const struct msg_t *msg, int msgnum,
                                        int options, pid_t pid)
{
    char expected_ident[IDENT_LENGTH];
    snprintf(expected_ident, sizeof(expected_ident), "%s%s%.0d%s:",
...
static struct msg_t parse_syslog_msg(const char *msg)
{
...
static struct msg_t parse_syslog_console(const char *msg)
{
    int priority;
    int facility;
    struct msg_t r;
...
static void check_syslog_udp(void (*syslog_send)(int), int options,
...
static void check_syslog_tcp(void (*syslog_send)(int), int options,
                             bool (*syslog_check)(const struct msg_t *, int, int,
                                     pid_t))
{
    struct sockaddr_un addr = {
...
static void check_syslog_console_read(FILE *fp)
{
    char buf[512];
    int msgnum = 0;
...
static void check_syslog_console_read_large(FILE *fp)
{
    char buf[2048];
...
static void check_syslog_console(void (*syslog_send)(int),
...
static void send_openlog_callback(void *clousure)
{
    int options = *(int *) clousure;
    send_openlog(options);
}

static void send_openlog_callback_large(void *clousure)
{
    int options = *(int *) clousure;
    send_openlog_large(options);
}

static void check_syslog_perror(bool large)
{
