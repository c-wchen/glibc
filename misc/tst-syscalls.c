...
struct Array allocate(size_t bytes)
{
    if (!bytes)
        return __extension__(struct Array) {
        0, 0
    };
...
__attribute_optimization_barrier__
void deallocate(struct Array b)
{
...
__attribute_optimization_barrier__
void *do_mmap(void *addr, size_t length)
{
...
__attribute_optimization_barrier__
void *reallocate(struct Array b)
{
    /* On x32, the 64-bit integer register containing `b' may be copied
       to another 64-bit integer register to pass the second argument to
       do_mmap.  */
...
__attribute_optimization_barrier__
ssize_t do_read(int fd, void *ptr, struct Array b)
{
    /* On x32, the 64-bit integer register containing `b' may be copied
       to another 64-bit integer register to pass the second argument to
       read.  */
...
__attribute_optimization_barrier__
ssize_t do_write(int fd, void *ptr, struct Array b)
{
    /* On x32, the 64-bit integer register containing `b' may be copied
       to another 64-bit integer register to pass the second argument to
       write.  */
