...
static void response_ptr(const struct resolv_response_context *ctx,
...
static void check_ai(const char *name, int ai_flags, const char *expected)
{
...
/* Run one getnameinfo test.  FLAGS is automatically augmented with
   NI_NUMERICSERV.  */
static void gni_test(enum gni_test code, unsigned int flags, const char *expected)
{
