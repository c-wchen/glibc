...
static void check_have_conf(void)
{
...
/* Verify that E matches the expected response for FAMILY and
   COUNTER.  */
static void check_hostent(const char *caller, const char *function, const char *qname,
...
static void check_addrinfo(const char *caller, const char *qname,
...
static void *thread_byname(void *closure)
{
    return byname(false);
}

/* Test gethostbyname2_r with AF_INET.  */
static void *thread_byname2(void *closure)
{
    return byname(true);
}

/* Test gethostbyname2_r with AF_INET6.  */
static void *thread_byname2_af_inet6(void *closure)
{
    int this_thread = get_thread_number();
    xpthread_barrier_wait(&barrier);
    for (int i = 0; i < queries_per_thread; ++i) {
...
static void *thread_gai_inet(void *closure)
{
    return gai(AF_INET);
}

/* Test getaddrinfo with AF_INET6.  */
static void *thread_gai_inet6(void *closure)
{
    return gai(AF_INET6);
}

/* Test getaddrinfo with AF_UNSPEC.  */
static void *thread_gai_unspec(void *closure)
{
