...
static void check_chroot_working(void *closure)
{
...
/* If FLAG is set in *OPTIONS, write NAME to FP, and clear it in
   *OPTIONS.  */
static void print_option_flag(FILE *fp, int *options, int flag, const char *name)
{
    if (*options & flag) {
        fprintf(fp, " %s", name);
        *options &= ~flag;
...
/* Write a decoded version of the resolver configuration *RESP to the
   stream FP.  */
static void print_resp(FILE *fp, res_state resp)
{
...
static void setup_nss_dns_and_chroot(void)
{
...
static void run_res_init(void *closure)
{
...
static void *run_res_init_thread_func(void *closure)
{
...
/* Variant of res_run_init which runs the function on a non-main
   thread.  */
static void run_res_init_on_thread(void *closure)
{
...
static void test_file_contents(const struct test_case *t)
{
#if TEST_THREAD
...
static void special_test_callback(void *closure)
{
...
/* Helper function which calls special_test_callback from a
   thread.  */
static void *special_test_thread_func(void *closure)
{
...
/* Variant of special_test_callback which runs the function on a
   non-main thread.  */
static void run_special_test_on_thread(void *closure)
{
...
static void special_test(unsigned int test_index)
{
#if TEST_THREAD
    for (int do_thread = 0; do_thread < 2; ++do_thread)
#endif
...
static pid_t start_dummy_server(void)
{
