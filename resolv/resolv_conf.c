...
static struct resolv_conf_global *get_locked_global(void)
{
...
static void put_locked_global(struct resolv_conf_global *global_copy)
{
    __libc_lock_unlock(lock);
}
...
static void conf_decrement(struct resolv_conf *conf)
{
...
/* Internal implementation of __resolv_conf_get, without validation
   against *RESP.  */
static struct resolv_conf *resolv_conf_get_1(const struct __res_state *resp)
{
...
static bool same_address_v4(const struct sockaddr_in *left,
...
/* Return true if both IPv6 addresses are equal.  This ignores the
   flow label.  */
static bool same_address_v6(const struct sockaddr_in6 *left,
...
static bool same_address(const struct sockaddr *left, const struct sockaddr *right)
{
...
static bool resolv_conf_matches(const struct __res_state *resp,
...
void __resolv_conf_put(struct resolv_conf *conf)
{
    if (conf == NULL) {
        return;
    }
...
/* Decrement the configuration object at INDEX and free it if the
   ...
   remains so.  */
static void decrement_at_index(struct resolv_conf_global *global_copy, size_t index)
{
    if (index < resolv_conf_array_size(&global_copy->array)) {
        /* Index found.  */
        uintptr_t *slot = resolv_conf_array_at(&global_copy->array, index);
...
bool __resolv_conf_attach(struct __res_state *resp, struct resolv_conf *conf)
{
    assert(conf->__refcount > 0);
...
void __resolv_conf_detach(struct __res_state *resp)
{
...
void __libc_resolv_conf_freemem(void)
{
