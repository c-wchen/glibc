...
static __always_inline char *inet_ntop4_format(const uint8_t *src, char *dst)
{
...
static __always_inline const char *inet_ntop4(const uint8_t *src, char *dst, socklen_t size)
{
...
static inline uint16_t in6_addr_addr16(const struct in6_addr *src, int idx)
{
    const struct {
        uint16_t x;
    } __attribute__((__packed__)) *pptr =
        (typeof(pptr))(&src->s6_addr16[idx]);
    return ntohs(pptr->x);
}

static __always_inline char *inet_ntop6_format(const struct in6_addr *src, struct best_t best, char *dst)
{
    char *tp = dst;
...
static inline const char *inet_ntop6(const struct in6_addr *src, char *dst, socklen_t size)
{
...
const char *__inet_ntop(int af, const void *src, char *dst, socklen_t size)
{
    switch (af) {
        case AF_INET:
