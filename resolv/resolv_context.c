...
static bool replicated_configuration_matches(const struct resolv_context *ctx)
{
...
/* Allocate a new context object and initialize it.  The object is put
   on the current list.  */
static struct resolv_context *context_alloc(struct __res_state *resp)
{
...
static void context_free(struct resolv_context *ctx)
{
...
static struct resolv_context *context_reuse(void)
{
...
/* Backing function for the __resolv_context_get family of
   functions.  */
static struct resolv_context *context_get(bool preinit)
{
    if (current != NULL) {
