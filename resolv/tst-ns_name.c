...
static bool base64_to_buffer(const char *base64, struct buffer *result)
{
...
static void free_test_case(struct test_case *t)
{
    free(t->path);
    free(t->input.data);
...
static bool parse_test_case(const char *path, size_t lineno, const char *line,
                            struct test_case *result)
{
    ...
    if (unpack_output == NULL)
        result->unpack_output = (struct buffer) {
        NULL, 0
    };
...
/* Format the buffer as a hexadecimal string and write it to standard
   output.  */
static void print_hex(const char *label, struct buffer buffer)
{
    printf("  %s ", label);
...
static void run_test_case(struct test_case *t)
{
    ...
    if (consumed != -1) {
        if (memcmp(unpacked, t->unpack_output.data,
            ...
            print_hex("actual:  ",
            (struct buffer) {
                unpacked, t->unpack_output.length
            });
            return;
        }
        ...
        if (ret != -1) {
            ...
            if (ret >= 0
                ...
                print_hex("actual:  ",
                (struct buffer) {
                    repacked, t->unpack_output.length
                });
                return;
            }
                ...
                if (ret < 0) {
                    ...
                    if (memcmp(repacked, unpacked, t->unpack_output.length) != 0) {
                        ...
                        printf("%s:%zu: error: wrong data from ns_name_compress\n",
                               t->path, t->lineno);
                        print_hex("expected:", t->unpack_output);
                        print_hex("actual:  ", (struct buffer) {
                            repacked, ret
                        });
                        return;
                    }
