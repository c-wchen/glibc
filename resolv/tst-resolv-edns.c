...
/* Extract information from the query, store it in a struct
   ...
   response_data_array.  */
static unsigned int put_response(const struct resolv_response_context *ctx,
...
/* Verify the index into the response_data array and return the data
   at it.  */
static struct response_data *get_response(unsigned int index)
{
    xpthread_mutex_lock(&mutex);
...
static void free_response_data(void)
{
    xpthread_mutex_lock(&mutex);
...
static void response(const struct resolv_response_context *ctx,
...
static struct response_data *decode_address(const void *address, size_t size)
{
    switch (size) {
        case 4:
...
static struct response_data *decode_hostent(struct hostent *e)
{
...
static struct response_data *decode_addrinfo(struct addrinfo *ai, int family)
{
...
static void verify_response_data_payload(struct response_data *data,
...
/* Same as verify_response_data_payload, but use the default
   payload.  */
static void verify_response_data(struct response_data *data)
{
    verify_response_data_payload(data, 1200);
}

static void check_hostent(struct hostent *e)
{
    TEST_VERIFY_EXIT(e != NULL);
    verify_response_data(decode_hostent(e));
}

static void do_ai(int family)
{
...
static const char *res_op_string(enum res_op op)
{
    switch (op) {
        case res_op_search:
            return "res_search";
...
static void do_res_search(const char *probe_name, enum res_op op, size_t size,
...
static void run_test(const char *probe_name)
{
    if (test_verbose)
