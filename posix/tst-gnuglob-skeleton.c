...
static struct DIRENT_STRUCT *my_readdir(void *gdir)
{
    my_DIR *dir = gdir;
...
static int my_stat(const char *name, struct STAT_STRUCT *st)
{
...
static const char *errstr(int val)
{
    static const char *const strs[] = {
...
static int test_result(const char *fmt, int flags, GLOB_TYPE *gl, const char *str[])
{
    size_t cnt;
    int result = 0;
