...
static void test_child(bool setgroup)
{
    if (setgroup) {
...
static void check_sigchld(int code, int status, pid_t pid)
{
#ifdef SA_SIGINFO
    siginfo_t siginfo;
...
static int do_test_waitd_common(idtype_t type, pid_t pid)
{
...
static int do_test_waitid(idtype_t type)
{
#ifdef SA_SIGINFO
    {
        struct sigaction sa;
