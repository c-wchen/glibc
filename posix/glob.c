...
static dirent_type readdir_result_type(struct readdir_result d)
{
#if defined _DIRENT_HAVE_D_TYPE || defined HAVE_STRUCT_DIRENT_D_TYPE
...
static struct readdir_result convert_dirent(const struct dirent *source)
{
    if (source == NULL) {
...
static struct readdir_result convert_dirent64(const struct dirent64 *source)
{
    if (source == NULL) {
        struct readdir_result result = { NULL, };
        return result;
...
static int glob_lstat(glob_t *pglob, int flags, const char *fullname)
{
...
static bool is_dir(char const *filename, int flags, glob_t const *pglob)
{
    struct_stat st;
    struct_stat64 st64;
    return (__glibc_unlikely(flags & GLOB_ALTDIRFUNC)
...
int
GLOB_ATTRIBUTE __glob(const char *pattern, int flags, int (*errfunc)(const char *, int),
                      glob_t *pglob)
{
    const char *filename;
    char *dirname = NULL;
