...
uint64_t __run_prefork_handlers(_Bool do_locking)
{
    uint64_t lastrun;
...
void __run_postfork_handlers(enum __run_fork_handler_type who, _Bool do_locking,
                             uint64_t lastrun)
{
    size_t sl = fork_handler_list_size(&fork_handlers);
    for (size_t i = 0; i < sl;) {
...
void __libc_atfork_freemem(void)
{
    lll_lock(atfork_lock, LLL_PRIVATE);
