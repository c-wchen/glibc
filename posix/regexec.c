...
int
attribute_compat_text_section __compat_regexec(const regex_t *__restrict preg,
...
regoff_t re_match(struct re_pattern_buffer *bufp, const char *string, Idx length,
...
static regoff_t re_search_stub(struct re_pattern_buffer *bufp, const char *string, Idx length,
...
static unsigned re_copy_regs(struct re_registers *regs, regmatch_t *pmatch, Idx nregs,
                             int regs_allocated)
{
...
__attribute_warn_unused_result__ re_search_internal(const regex_t *preg, const char *string, Idx length,
...
static reg_errcode_t
__attribute_warn_unused_result__ prune_impossible_nodes(re_match_context_t *mctx)
{
...
static Idx
__attribute_warn_unused_result__ check_matching(re_match_context_t *mctx, bool fl_longest_match,
        Idx *p_match_first)
{
    const re_dfa_t *const dfa = mctx->dfa;
    reg_errcode_t err;
...
static Idx proceed_next_node(const re_match_context_t *mctx, Idx nregs, regmatch_t *regs,
                             regmatch_t *prevregs,
...
static reg_errcode_t
__attribute_warn_unused_result__ push_fail_stack(struct re_fail_stack_t *fs, Idx str_idx, Idx dest_node,
...
static Idx pop_fail_stack(struct re_fail_stack_t *fs, Idx *pidx, Idx nregs,
                          regmatch_t *regs, regmatch_t *prevregs,
                          re_node_set *eps_via_nodes)
{
    if (fs == NULL || fs->num == 0) {
...
static reg_errcode_t
__attribute_warn_unused_result__ set_regs(const regex_t *preg, const re_match_context_t *mctx, size_t nmatch,
...
static reg_errcode_t
__attribute_warn_unused_result__ build_sifted_states(const re_match_context_t *mctx, re_sift_context_t *sctx,
...
static reg_errcode_t merge_state_array(const re_dfa_t *dfa, re_dfastate_t **dst,
                                       re_dfastate_t **src, Idx num)
{
    Idx st_idx;
    reg_errcode_t err;
...
static reg_errcode_t
__attribute_warn_unused_result__ add_epsilon_src_nodes(const re_dfa_t *dfa, re_node_set *dest_nodes,
        const re_node_set *candidates)
{
    reg_errcode_t err = REG_NOERROR;
    Idx i;
...
static reg_errcode_t sub_epsilon_src_nodes(const re_dfa_t *dfa, Idx node, re_node_set *dest_nodes,
        const re_node_set *candidates)
{
    Idx ecl_idx;
    reg_errcode_t err;
...
static bool check_dst_limits(const re_match_context_t *mctx, const re_node_set *limits,
...
static int check_dst_limits_calc_pos_1(const re_match_context_t *mctx, int boundaries,
...
static int check_dst_limits_calc_pos(const re_match_context_t *mctx, Idx limit,
...
static reg_errcode_t
__attribute_warn_unused_result__ sift_states_bkref(const re_match_context_t *mctx, re_sift_context_t *sctx,
        Idx str_idx, const re_node_set *candidates)
{
    const re_dfa_t *const dfa = mctx->dfa;
    reg_errcode_t err;
...
#ifdef RE_ENABLE_I18N
static int sift_states_iter_mb(const re_match_context_t *mctx, re_sift_context_t *sctx,
                               Idx node_idx, Idx str_idx, Idx max_str_idx)
{
    const re_dfa_t *const dfa = mctx->dfa;
    int naccepted;
...
__attribute_warn_unused_result__ transit_state(reg_errcode_t *err, re_match_context_t *mctx,
        re_dfastate_t *state)
{
...
static re_dfastate_t *transit_state_sb(reg_errcode_t *err, re_match_context_t *mctx,
                                       re_dfastate_t *state)
{
    const re_dfa_t *const dfa = mctx->dfa;
    re_node_set next_nodes;
...
static reg_errcode_t
__attribute_warn_unused_result__ get_subexp(re_match_context_t *mctx, Idx bkref_node, Idx bkref_str_idx)
{
    const re_dfa_t *const dfa = mctx->dfa;
...
static reg_errcode_t get_subexp_sub(re_match_context_t *mctx, const re_sub_match_top_t *sub_top,
...
static reg_errcode_t
__attribute_warn_unused_result__ check_arrival(re_match_context_t *mctx, state_array_t *path, Idx top_node,
...
static reg_errcode_t
__attribute_warn_unused_result__ check_arrival_add_next_nodes(re_match_context_t *mctx, Idx str_idx,
...
static reg_errcode_t check_arrival_expand_ecl(const re_dfa_t *dfa, re_node_set *cur_nodes,
        Idx ex_subexp, int type)
{
    reg_errcode_t err;
...
static reg_errcode_t
__attribute_warn_unused_result__ check_arrival_expand_ecl_sub(const re_dfa_t *dfa, re_node_set *dst_nodes,
...
static reg_errcode_t
__attribute_warn_unused_result__ expand_bkref_cache(re_match_context_t *mctx, re_node_set *cur_nodes,
...
static bool __attribute_noinline__ build_trtable(const re_dfa_t *dfa, re_dfastate_t *state)
{
    reg_errcode_t err;
    Idx i, j;
    int ch;
...
static Idx group_nodes_into_DFAstates(const re_dfa_t *dfa, const re_dfastate_t *state,
...
# ifdef _LIBC
static unsigned int find_collation_sequence_value(const unsigned char *mbs, size_t mbs_len)
{
...
static bool check_node_accept(const re_match_context_t *mctx, const re_token_t *node,
                              Idx idx)
{
    unsigned char ch;
...
static reg_errcode_t
__attribute_warn_unused_result__ extend_buffers(re_match_context_t *mctx, int min_len)
{
    reg_errcode_t ret;
...
static reg_errcode_t
__attribute_warn_unused_result__ match_ctx_init(re_match_context_t *mctx, int eflags, Idx n)
{
    mctx->eflags = eflags;
    mctx->match_last = -1;
    if (n > 0) {
...
static reg_errcode_t
__attribute_warn_unused_result__ match_ctx_add_entry(re_match_context_t *mctx, Idx node, Idx str_idx, Idx from,
        Idx to)
{
...
static reg_errcode_t
__attribute_warn_unused_result__ match_ctx_add_subtop(re_match_context_t *mctx, Idx node, Idx str_idx)
{
...
static re_sub_match_last_t *match_ctx_add_sublast(re_sub_match_top_t *subtop, Idx node, Idx str_idx)
{
