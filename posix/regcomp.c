...
static void re_compile_fastmap_iter(regex_t *bufp, const re_dfastate_t *init_state,
                                    char *fastmap)
{
    re_dfa_t *dfa = bufp->buffer;
    Idx node_cnt;
...
#ifdef _LIBC
void __libc_regcomp_freemem(void)
{
    __regfree(&re_comp_buf);
}
#endif
...
static reg_errcode_t re_compile_internal(regex_t *preg, const char *pattern, size_t length,
        reg_syntax_t syntax)
{
...
static bin_tree_t *duplicate_tree(const bin_tree_t *root, re_dfa_t *dfa)
{
