...
static void print_cpu_feature_internal(unsigned int index, const char *kind,
...
static void print_cpu_feature_preferred(const char *label, unsigned int flag)
{
...
/* Single integer value that can be used for sorting/ordering
   ...
   subsequent one.  */
static inline unsigned long long int cpuid_query_combined(struct cpuid_query *q)
{
...
static bool _dl_diagnostics_cpuid_store(struct cpuid_collected_data *ccd,
...
/* Collected CPUID data into *CCD.  If LIMIT, apply per-leaf limits to
   ...
   requested.  */
static bool _dl_diagnostics_cpuid_collect_1(struct cpuid_collected_data *ccd, bool limit)
{
    ccd->used = 0;
    ccd->ecx_limit
...
static void _dl_diagnostics_cpuid_collect(struct cpuid_collected_data *ccd)
{
...
static void _dl_diagnostics_cpuid_print_reg(unsigned int processor_index,
...
static void _dl_diagnostics_cpuid_print_query(unsigned int processor_index,
...
static void _dl_diagnostics_cpuid_report(struct dl_iterate_cpu *dci,
