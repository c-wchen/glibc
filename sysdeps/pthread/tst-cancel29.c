...
static void *tf_sigtimedwait(void *arg)
{
    ...
    sigset_t mask;
    sigemptyset(&mask);
    r = sigtimedwait(&mask, NULL, &(struct timespec) {
        0, 250000000
    });
    if (r != -1) {
        return (void *) -1;
...
static void *tf_ppoll(void *arg)
{
    ...
    r = ppoll(NULL, 0, &(struct timespec) {
        0, 250000000
    }, NULL);
    if (r != 0) {
        return (void *) -1;
...
static void *tf_select(void *arg)
{
    ...
    r = select(0, NULL, NULL, NULL, &(struct timeval) {
        0, 250000
    });
    if (r != 0) {
        return (void *) -1;
...
static void *tf_pselect(void *arg)
{
    ...
    r = pselect(0, NULL, NULL, NULL, &(struct timespec) {
        0, 250000000
    }, NULL);
    if (r != 0) {
        return (void *) -1;
...
static void *tf_clock_nanosleep(void *arg)
{
    pthread_setcancelstate(PTHREAD_CANCEL_DISABLE, NULL);
    xpthread_barrier_wait(&b);
    ...
    r = clock_nanosleep(CLOCK_REALTIME, 0, &(struct timespec) {
        0, 250000000
    },
    NULL);
    if (r != 0) {
