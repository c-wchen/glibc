...
static size_t trim_prefix(char *str, size_t str_len, const char *prefix)
{
...
static char *proc_fd_readlink(const char *linkname)
{
...
static bool eq_ttyname(struct result actual, struct result expected)
{
...
/* strings in result structure are in static storage */
static struct result_r run_ttyname_r(int fd)
{
...
static bool eq_ttyname_r(struct result_r actual, struct result_r expected)
{
    char *actual_name, *expected_name;
...
static bool doit(int fd, const char *testname, struct result_r expected_r)
{
...
static void adjust_file_limit(const char *pty)
{
    int number = -1;
...
static int run_chroot_tests(const char *slavename, int slave)
{
    struct stat st;
    bool ok = true;
