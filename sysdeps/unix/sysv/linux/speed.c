...
speed_t ___cbaud_to_speed(tcflag_t c_cflag, speed_t other)
{
...
tcflag_t ___speed_to_cbaud(speed_t speed)
{
    switch (speed) {
        case 0:
            return __B0;
...
void ___termios2_canonicalize_speeds(struct termios2 *k_termios_p)
{
...
speed_t __cfgetospeed(const struct termios *termios_p)
{
...
speed_t __cfgetispeed(const struct termios *termios_p)
{
...
int __cfsetospeed(struct termios *termios_p, speed_t speed)
{
...
int __cfsetispeed(struct termios *termios_p, speed_t speed)
{
    tcflag_t cbaud = ___speed_to_cbaud(speed);
...
speed_t
attribute_compat_text_section __old_cfgetospeed(const old_termios_t *termios_p)
{
...
speed_t
attribute_compat_text_section __old_cfgetispeed(const old_termios_t *termios_p)
{
...
int
attribute_compat_text_section __old_cfsetospeed(old_termios_t *termios_p, speed_t speed)
{
...
int
attribute_compat_text_section __old_cfsetispeed(old_termios_t *termios_p, speed_t speed)
{
    speed_t real_speed = ___cbaud_to_speed(speed, -1);
    if (real_speed == (speed_t) -1) {
        return INLINE_SYSCALL_ERROR_RETURN_VALUE(EINVAL);
