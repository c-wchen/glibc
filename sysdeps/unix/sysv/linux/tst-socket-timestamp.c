...
static int do_sendto(const struct sockaddr_in *addr, int nmsgs)
{
...
static void do_recvmsg_slack_ancillary(bool use_multi_call, int s, void *cmsg,
...
static void do_test_slack_space(void)
{
    ...
    {
        int r = setsockopt(srv, SOL_SOCKET, SO_TIMESTAMP, &(int) {
            1
        },
        sizeof(int));
    ...
    /* Now enable timestamp using a higher precision, it overwrites the previous
       precision.  */
    {
        int r = setsockopt(srv, SOL_SOCKET, SO_TIMESTAMPNS, &(int) {
            1
        },
        sizeof(int));
        TEST_VERIFY_EXIT(r != -1);
...
static void do_recvmsg_multiple_ancillary(bool use_multi_call, int s, void *cmsg,
...
static void do_test_multiple_ancillary(void)
{
    {
        int r = setsockopt(srv, SOL_SOCKET, SO_TIMESTAMP, &(int) {
            1
    ...
    {
        int r = setsockopt(srv, IPPROTO_IP, IP_RECVORIGDSTADDR, &(int) {
            1
        },
        sizeof(int));
        TEST_VERIFY_EXIT(r != -1);
