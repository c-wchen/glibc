...
static void read_sem_stat(struct test_seminfo *tseminfo)
{
...
static bool check_seminfo(int idx, key_t key, int cmd)
{
    struct semid_ds seminfo;
    int sid = semctl(idx, 0, cmd, (union semun) {
        .buf = &seminfo
    });
...
static int do_test(void)
{
...

    {
        ...
        semidx = semctl(semid, 0, IPC_INFO, (union semun) {
            .__buf = &ipcinfo
        });
        if (semidx == -1) {
    ...
    {
        struct seminfo ipcinfo;
        semidx = semctl(semid, 0, SEM_INFO, (union semun) {
            .__buf = &ipcinfo
        });
        if (semidx == -1) {
            FAIL_EXIT1("semctl with IPC_INFO failed: %m");
