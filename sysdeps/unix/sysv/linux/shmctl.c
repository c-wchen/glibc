...
_Static_assert(sizeof((struct shmid_ds)
{
    0
} .shm_perm.mode)
== sizeof(mode_t),
...
static void shmid64_to_kshmid64(const struct __shmid64_ds *shmid64,
...
static void kshmid64_to_shmid64(const struct kernel_shmid64_ds *kshmid,
...
static int shmctl_syscall(int shmid, int cmd, shmctl_arg_t *buf)
{
...
int
attribute_compat_text_section __shmctl_mode16(int shmid, int cmd, struct shmid_ds *buf)
{
...
int
attribute_compat_text_section __old_shmctl(int shmid, int cmd, struct __old_shmid_ds *buf)
{
