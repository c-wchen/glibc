...
static int __get_nprocs_sched(void)
{
    enum {
        max_num_cpus = 32768,
...
static char *next_line(int fd, char *const buffer, char **cp, char **re,
...
static int get_nproc_stat(void)
{
...
static int read_sysfs_file(const char *fname)
{
    enum { buffer_size = 1024 };
    char buffer[buffer_size];
    char *buffer_end = buffer + buffer_size;
...
static int get_nprocs_fallback(void)
{
    int result;
