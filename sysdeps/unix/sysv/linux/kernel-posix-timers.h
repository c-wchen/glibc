...
static inline timer_t kernel_timer_to_timerid(kernel_timer_t ktimerid)
{
    return (timer_t)((intptr_t) ktimerid);
}

static inline timer_t timer_to_timerid(struct timer *ptr)
{
    return (timer_t)(INTPTR_MIN | (uintptr_t) ptr >> 1);
}

static inline bool timer_is_sigev_thread(timer_t timerid)
{
    return (intptr_t) timerid < 0;
}

static inline struct timer *timerid_to_timer(timer_t timerid)
{
    return (struct timer *)((uintptr_t) timerid << 1);
}

static inline kernel_timer_t timerid_to_kernel_timer(timer_t timerid)
{
