...
static void send_fd(const int sock, const int fd)
{
    struct msghdr msg = {0};
    union {
        struct cmsghdr hdr;
...
static int recv_fd(const int sock)
{
    struct msghdr msg = {0};
    union {
        struct cmsghdr hdr;
...
static int child_func(void *const arg)
{
    xclose(sockfd[0]);
...
static void update_map(char *const mapping, const char *const map_file)
{
...
static void proc_setgroups_write(const long child_pid, const char *const str)
{
