...
static void semid64_to_ksemid64(const struct __semid64_ds *semid64,
...
static void ksemid64_to_semid64(const struct kernel_semid64_ds *ksemid,
...
static union ksemun64 semun64_to_ksemun64(int cmd, union semun64 semun64,
            struct kernel_semid64_ds *buf)
{
    union ksemun64 r = { 0 };
    switch (cmd) {
...
static int semctl_syscall(int semid, int semnum, int cmd, semctl_arg_t arg)
{
...
_Static_assert(sizeof((struct semid_ds)
{
    0
} .sem_perm.mode)
== sizeof(mode_t),
...
int __semctl64(int semid, int semnum, int cmd, ...)
{
...
static void semid64_to_semid(struct semid_ds *ds, const struct __semid64_ds *ds64)
{
...
static union semun64 semun_to_semun64(int cmd, union semun semun, struct __semid64_ds *semid64)
{
    union semun64 r = { 0 };
    switch (cmd) {
        case SETVAL:
...
int
attribute_compat_text_section __semctl_mode16(int semid, int semnum, int cmd, ...)
{
    semctl_arg_t arg = { 0 };
    va_list ap;
...
/* Since semctl use a variadic argument for semid_ds there is not need to
   ...
   definition.  */
int
attribute_compat_text_section __old_semctl(int semid, int semnum, int cmd, ...)
{
    union semun arg = { 0 };
    va_list ap;
