...
static int find_last_cpu(const cpu_set_t *set, size_t size)
{
...
static void setup_conf(struct conf *conf)
{
    *conf = (struct conf) {
        -1, -1
        };
...
static bool test_size(const struct conf *conf, size_t size)
{
