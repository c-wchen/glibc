...
static float64x2_t NOINLINE VPCS_ATTR special_case(float64x2_t x, float64x2_t y, uint32x2_t special)
{
...
VPCS_ATTR float64x2_t V_NAME_D1(cbrt)(float64x2_t x)
{
    ...
    float64x2_t my = (float64x2_t) {
        d->table[em3[0] + 2], d->table[em3[1] + 2]
    };
    my = vmulq_f64(my, a);
