...
static inline double special_case(double tmp, uint64_t sbits, uint64_t ki)
{
    double scale, y;
...
/* Computes exp(x+xtail) where |xtail| < 2^-8/N and |xtail| <= |x|.
   ...
   equal to 0.  */
static double NOINLINE exp_nosignbias(double x, double xtail)
{
    uint32_t abstop = top12(x) & 0x7ff;
    if (__glibc_unlikely(abstop - SmallExp >= ThresExp)) {
        /* Avoid spurious underflow for tiny x.  */
...
static double NOINLINE pow_scalar_special_case(double x, double y)
{
