...
static float64x2_t NOINLINE VPCS_ATTR special_case(float64x2_t x, float64x2_t y, uint64x2_t abs_mask,
        uint64x2_t special)
{
    /* Copy sign.  */
...
static inline struct entry lookup(uint64x2_t i)
{
...
static inline float64x2_t log_inline(float64x2_t xm, const struct data *d)
{
