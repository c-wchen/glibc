...
static const struct data {
    ...
    /* rel err: 1.5 * 2^-30. Each coefficients is multiplied the value of
       V_POWF_EXP2_N.  */
    .log_poly = {
        -0x1.6ff5daa3b3d7cp + 3, 0x1.ec81d03c01aebp + 3,
...
static inline svbool_t svisint(svbool_t pg, svfloat32_t x)
{
    return svcmpeq(pg, svrintz_z(pg, x), x);
}

/* Check if x is real not integer valued.  */
static inline svbool_t svisnotint(svbool_t pg, svfloat32_t x)
{
    return svcmpne(pg, svrintz_z(pg, x), x);
}

/* Check if x is an odd integer.  */
static inline svbool_t svisodd(svbool_t pg, svfloat32_t x)
{
...
static inline svbool_t sv_zeroinfnan(svbool_t pg, svuint32_t i)
{
...
static inline float powf_specialcase(float x, float y, float z)
{
...
static svfloat32_t NOINLINE sv_call_powf_sc(svfloat32_t x1, svfloat32_t x2, svfloat32_t y)
{
...
static inline svfloat64_t sv_powf_core_ext(const svbool_t pg, svuint64_t i, svfloat64_t z, svint64_t k,
...
static inline svfloat32_t sv_powf_core(const svbool_t pg, svuint32_t i, svuint32_t iz, svint32_t k,
