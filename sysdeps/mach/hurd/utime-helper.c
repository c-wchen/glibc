...
static inline bool check_tval(const struct timeval *tvp)
{
    return tvp->tv_usec >= 0 && tvp->tv_usec < USEC_PER_SEC;
}

/* Initializes atime/mtime timespec structures from an array of timeval.  */
static inline error_t utime_ts_from_tval(const struct timeval tvp[2],
...
static inline error_t utime_tvalue_from_tval(const struct timeval tvp[2],
...
static inline error_t hurd_futimes(const file_t port, const struct timeval tvp[2])
{
    error_t err;
...
static inline bool check_tspec(const struct timespec *tsp)
{
...
static inline error_t utime_ts_from_tspec(const struct timespec tsp[2],
        struct timespec *atime, struct timespec *mtime)
{
    if (tsp == NULL) {
        /* Setting the number of nanoseconds to UTIME_NOW tells the
...
static inline error_t utime_tvalue_from_tspec(const struct timespec tsp[2],
        time_value_t *atime, time_value_t *mtime)
{
    if (tsp == NULL)
        /* Setting the number of microseconds to `-1' tells the
...
static inline error_t hurd_futimens(const file_t port, const struct timespec tsp[2])
{
    error_t err;
    struct timespec atime, mtime;
