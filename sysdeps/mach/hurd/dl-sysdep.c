...
static error_t open_file(const char *file_name, int flags,
...
__ssize_t weak_function __pread64(int fd, void *buf, size_t nbytes, off64_t offset)
{
    error_t err;
    char *data;
...
__ssize_t weak_function __read(int fd, void *buf, size_t nbytes)
{
...
__ssize_t weak_function __write(int fd, const void *buf, size_t nbytes)
{
    error_t err;
    vm_size_t nwrote;
...
__ssize_t weak_function __writev(int fd, const struct iovec *iov, int niov)
{
...
void *weak_function __mmap(void *addr, size_t len, int prot, int flags, int fd, off_t offset)
{
    error_t err;
    vm_prot_t vmprot;
...
int weak_function __fstat64(int fd, struct stat64 *buf)
{
    error_t err;
...
int weak_function __stat64(const char *file, struct stat64 *buf)
{
    error_t err;
    mach_port_t port;
...
int weak_function __access(const char *file, int type)
{
    return __hurd_fail(ENOSYS);
}

int __rtld_execve(const char *file_name, char *const argv[],
                  char *const envp[])
{
    file_t file;
    error_t err;
...
pid_t weak_function __getpid(void)
{
    pid_t pid, ppid;
    int orphaned;
...
void *weak_function __sbrk(intptr_t increment)
{
    vm_address_t addr;
...
void weak_function _dl_init_first(void *p)
{
