...
static float j1f_asympt(float x)
{
...
static float j1f_near_root(float x, float z)
{
...
/* Formula page 5 of https://www.cl.cam.ac.uk/~jrh13/papers/bessel.pdf:
   ...
   where beta1(x) = 1 + 3/(16*x^2) - 99/(512*x^4)
   and alpha1(x) = -3/(8*x) + 21/(128*x^3) - 1899/(5120*x^5).  */
static float y1f_asympt(float x)
{
    float cst = 0xc.c422ap - 4; /* sqrt(2/pi) rounded to nearest  */
    double y = 1.0 / (double) x;
    double y2 = y * y;
...
static float y1f_near_root(float x, float z)
{
    float index_f;
    int index;
...
static float qonef(float x)
{
    ...
    if (ix >= 0x41000000)     {
        ...
        p = qr5;    /* x >= 7.722209930e+00  */
        q = qs5;
    } else if (ix >= 0x4036db68) {
        p = qr3;    /* x >= 2.857141495e+00  */
        q = qs3;
    } else {
        p = qr2;    /* x >= 2  */
        q = qs2;
