...
static inline int clz_uint64(uint64_t x)
{
...
static inline int ctz_uint64(uint64_t x)
{
    if (sizeof(uint64_t) == sizeof(unsigned long)) {
...
static inline bool is_nan(uint64_t x)
{
    return (x & EXP_MANT_MASK) > EXPONENT_MASK;
}

static inline bool is_inf(uint64_t x)
{
    return (x << 1) == (EXPONENT_MASK << 1);
}

static inline uint64_t get_mantissa(uint64_t x)
{
    return x & MANTISSA_MASK;
}

static inline int get_exponent(uint64_t x)
{
...
static inline double make_double(uint64_t x, int64_t ep, uint64_t s)
{
