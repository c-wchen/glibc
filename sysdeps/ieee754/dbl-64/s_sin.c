...
/* Given a number partitioned into X and DX, this function computes the cosine
   ...
   get the result.  */
static __always_inline double do_cos(double x, double dx)
{
    mynumber u;
...
/* Reduce range of x to within PI/2 with abs (x) < 105414350.  The high part
   ...
   are correct.  */
static __always_inline int4 reduce_sincos(double x, double *a, double *da)
{
    mynumber v;
...
static __always_inline double do_sincos(double a, double da, int4 n)
{
    double retval;
