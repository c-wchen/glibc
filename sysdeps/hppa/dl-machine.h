...
static inline void __hppa_init_bootstrap_fdesc_table(struct link_map *map)
{
...
static inline Elf32_Addr elf_machine_load_address(void) __attribute__((const));
...
static inline struct fdesc elf_machine_plt_value(struct link_map *map, const Elf32_Rela *reloc,
        struct fdesc value)
{
    /* We are rela only, return a function descriptor as a plt entry. */
    return (struct fdesc) {
        value.ip + reloc->r_addend, value.gp
    };
}
...
static inline int elf_machine_runtime_setup(struct link_map *l, struct r_scope_elem *scope[],
