...
/* Returns the soname string if the link map has a DT_SONAME tag, or
   NULL if it does not.  */
static inline const char *l_soname(const struct link_map *l)
{
...
static __always_inline bool dl_symbol_visibility_binds_local_p(const ElfW(Sym) *sym)
{
...
static inline bool is_rtld_link_map(const struct link_map *l)
{
    return l == &_dl_rtld_map;
}

static inline struct auditstate *link_map_audit_state(struct link_map *l, size_t index)
{
...
static inline void _dl_audit_objclose(struct link_map *l)
{
