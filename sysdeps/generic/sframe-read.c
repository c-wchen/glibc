...
static inline uint32_t sframe_get_hdr_size(sframe_header *sfh)
{
...
static inline uint8_t sframe_fre_get_offset_count(uint8_t fre_info)
{
    return SFRAME_V1_FRE_OFFSET_COUNT(fre_info);
}

static inline uint8_t sframe_fre_get_offset_size(uint8_t fre_info)
{
    return SFRAME_V1_FRE_OFFSET_SIZE(fre_info);
}

static inline bool sframe_get_fre_ra_mangled_p(uint8_t fre_info)
{
...
static uint32_t sframe_get_fre_type(sframe_func_desc_entry *fdep)
{
    uint32_t fre_type = 0;
    if (fdep != NULL) {
...
static uint32_t sframe_get_fde_type(sframe_func_desc_entry *fdep)
{
    uint32_t fde_type = 0;
    if (fdep != NULL) {
...
static bool sframe_header_sanity_check_p(sframe_header *hp)
{
...
static size_t sframe_fre_start_addr_size(uint32_t fre_type)
{
    size_t addr_size = 0;
    switch (fre_type) {
...
static bool sframe_fre_sanity_check_p(sframe_frame_row_entry *frep)
{
...
static size_t sframe_fre_offset_bytes_size(uint8_t fre_info)
{
    uint8_t offset_size, offset_cnt;
...
static size_t sframe_fre_entry_size(sframe_frame_row_entry *frep, size_t addr_size)
{
    if (frep == NULL) {
        return 0;
    }
...
static inline sframe_header *sframe_decoder_get_header(sframe_decoder_ctx *dctx)
{
...
static uint32_t sframe_decoder_get_offsetof_fde_start_addr(sframe_decoder_ctx *dctx,
        uint32_t func_idx,
...
static int32_t sframe_decoder_get_secrel_func_start_addr(sframe_decoder_ctx *dctx,
        uint32_t func_idx)
{
...
static bool sframe_fre_check_range_p(sframe_decoder_ctx *dctx, uint32_t func_idx,
...
static int32_t sframe_get_fre_offset(sframe_frame_row_entry *fre,
                                     int idx,
                                     _Unwind_Reason_Code *errp)
{
...
static void sframe_decode_fre_start_address(const char *fre_buf,
...
static sframe_func_desc_entry *sframe_get_funcdesc_with_addr_internal(sframe_decoder_ctx *ctx, int32_t addr,
...
static uint32_t sframe_fre_get_end_ip_offset(sframe_func_desc_entry *fdep, unsigned int i,
        const char *fres)
{
...
static int8_t sframe_decoder_get_fixed_fp_offset(sframe_decoder_ctx *ctx)
{
    sframe_header *dhp;
    dhp = sframe_decoder_get_header(ctx);
...
static int8_t sframe_decoder_get_fixed_ra_offset(sframe_decoder_ctx *ctx)
{
    sframe_header *dhp;
    dhp = sframe_decoder_get_header(ctx);
...
uint8_t __sframe_fre_get_base_reg_id(sframe_frame_row_entry *fre)
{
...
int32_t __sframe_fre_get_cfa_offset(sframe_decoder_ctx *dctx __attribute__((__unused__)),
...
int32_t __sframe_fre_get_fp_offset(sframe_decoder_ctx *dctx,
                                   sframe_frame_row_entry *fre,
                                   _Unwind_Reason_Code *errp)
{
...
int32_t __sframe_fre_get_ra_offset(sframe_decoder_ctx *dctx,
                                   sframe_frame_row_entry *fre,
                                   _Unwind_Reason_Code *errp)
{
...
_Unwind_Reason_Code __sframe_decode(sframe_decoder_ctx *dctx, const char *sf_buf)
{
...
_Unwind_Reason_Code __sframe_find_fre(sframe_decoder_ctx *ctx, int32_t pc,
