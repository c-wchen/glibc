...
static __always_inline int riscv_getround(void)
{
    return get_rounding_mode();
}

static __always_inline void riscv_setround(int rm)
{
    asm volatile("fsrm %z0" : : "rJ"(rm));
}

static __always_inline int riscv_getflags(void)
{
    int flags;
...
static __always_inline void riscv_setflags(int flags)
{
    asm volatile("fsflags %z0" : : "rJ"(flags));
}

static __always_inline void libc_feholdexcept_riscv(fenv_t *envp)
{
...
static __always_inline void libc_fesetround_riscv(int round)
{
...
static __always_inline void libc_feholdexcept_setround_riscv(fenv_t *envp, int round)
{
...
static __always_inline int libc_fetestexcept_riscv(int ex)
{
...
static __always_inline void libc_fesetenv_riscv(const fenv_t *envp)
{
...
static __always_inline int libc_feupdateenv_test_riscv(const fenv_t *envp, int ex)
{
    fenv_t env = *envp;
...
static __always_inline void libc_feupdateenv_riscv(const fenv_t *envp)
{
    long int env = (envp != FE_DFL_ENV ? *envp : 0);
...
static __always_inline void libc_feholdsetround_riscv(fenv_t *envp, int round)
{
...
static __always_inline void libc_feresetround_riscv(fenv_t *envp)
{
