...
static struct in6_addr from_string(const char *address)
{
...
static bool call_gai(int family, const char *address, const char *scope,
...
/* Verify that a successful call to getaddrinfo returned the expected
   scope data.  */
static void check_ai(const char *what, const char *addr_string, const char *scope_string,
...
static void expect_failure(const char *address, const char *scope)
{
    if (test_verbose > 0) {
...
static void expect_success(const char *address, const char *scope, uint32_t expected)
{
    if (test_verbose > 0) {
