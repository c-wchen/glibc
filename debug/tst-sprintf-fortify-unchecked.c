...
static int my_vsprintf(char *buf, const char *format, va_list ap)
{
    int result;
    if (fortify_mode == 0) {
...
/* printf callback that falls back to the glibc-supplied
   implementation.  */
static int dummy_printf_function(FILE *__stream,
...
static int dummy_arginfo_function(const struct printf_info *info,
