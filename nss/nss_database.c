...
static void *global_state_allocate(void *closure)
{
...
static struct nss_database_state *nss_database_state_get(void)
{
...
static bool nss_database_select_default(struct nss_database_default_cache *cache,
...
static int name_search(const void *left, const void *right)
{
    return strcmp(left, right);
}

static int name_to_database_index(const char *name)
{
...
static bool process_line(struct nss_database_data *data, char *line)
{
...
int __nss_configure_lookup(const char *dbname, const char *service_line)
{
    int db;
...
static bool nss_database_reload_1(struct nss_database_data *data, FILE *fp)
{
    char *line = NULL;
...
static bool nss_database_reload(struct nss_database_data *staging,
...
static bool nss_database_check_reload_and_get(struct nss_database_state *local,
        nss_action_list *result,
