...
static bool module_load_builtin(struct nss_module *module,
...
static bool module_load_nss_files(struct nss_module *module)
{
#if defined SHARED && defined USE_NSCD
    if (is_nscd) {
...
static bool module_load_nss_dns(struct nss_module *module)
{
    return module_load_builtin(module, __nss_dns_functions);
}

/* Internal implementation of __nss_module_load.  */
static bool module_load(struct nss_module *module)
{
...
/* Force the module identified by MODULE to be loaded.  We return
   ...
   caching the results.  */
bool __nss_module_load(struct nss_module *module)
{
    switch ((enum nss_module_state) atomic_load_acquire(&module->state)) {
        case nss_module_uninitialized:
...
static void nss_load_all_libraries(enum nss_database service)
{
...
void __nss_disable_nscd(void (*cb)(size_t, struct traced_file *))
{
