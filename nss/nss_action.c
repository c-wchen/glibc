...
/* Returns true if the actions are equal (same module, same actions
   array).  */
static bool actions_equal(const struct nss_action *a, const struct nss_action *b)
{
...
/* Returns true if COUNT actions at A and B are equal (according to
   ...
   least COUNT actions.  */
static bool action_lists_equal(const struct nss_action *a, const struct nss_action *b,
                               size_t count)
{
...
static nss_action_list find_allocated(struct nss_action *actions, size_t count)
{
...
nss_action_list __nss_action_allocate(struct nss_action *actions, size_t count)
{
