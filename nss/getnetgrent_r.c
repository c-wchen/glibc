...
/* Set up NIP to run through the services.  Return nonzero if there are no
   services (left).  */
static int setup(void **fctp, nss_action_list *nipp)
{
    int no_more;
...
static int __internal_setnetgrent_reuse(const char *group, struct __netgrent *datap,
                                        int *errnop)
{
    union {
...
int setnetgrent(const char *group)
{
    int result;
...
#ifdef USE_NSCD
static const char *get_nonempty_val(const char *in)
{
    if (*in == '\0') {
        return NULL;
    }
...
int __internal_getnetgrent_r(char **hostp, char **userp, char **domainp,
