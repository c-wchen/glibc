...
/* Initialize the NSS interface/functions. The calling function must
   hold the lock.  */
static void init_nss_interface(void)
{
...
static void give_pwd_free(struct passwd *pwd)
{
    free(pwd->pw_name);
    free(pwd->pw_passwd);
    free(pwd->pw_gecos);
...
static size_t pwd_need_buflen(struct passwd *pwd)
{
    size_t len = 0;
...
static void copy_pwd_changes(struct passwd *dest, struct passwd *src,
                             char *buffer, size_t buflen)
{
...
static void internal_endpwent_noerror(ent_t *ent)
{
...
static void blacklist_store_name(const char *name, ent_t *ent)
{
...
static bool in_blacklist(const char *name, int namelen, ent_t *ent)
{
    char buf[namelen + 3];
    char *cp;
