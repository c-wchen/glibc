...
static int dbentry_compare(const void *p1, const void *p2)
{
...
static int valstr_compare(const void *p1, const void *p2)
{
...
static int process_input(FILE *input, const char *inname, int to_lowercase, int be_quiet)
{
    char *line;
    size_t linelen;
    int status;
    ...
    while (!feof_unlocked(input)) {
        ...
        *cp++ = '\0';
        ...
        if (*fdata != nentry) {
            ...
            free(nentry);
            nentry = *fdata;
        } else if (last_database->extra_string) {
            extrastrlen += datalen;
        } else {
            valstrlen += datalen;
        }
...
static void copy_valstr(const void *nodep, const VISIT which, const int depth)
{
...
static size_t next_prime(size_t seed)
{
