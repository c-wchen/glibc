...
static bool equal(const struct passwd *a, const struct passwd *b)
{
...
/* Initialize test_items and test_count above, with data from the
   passwd database.  */
static bool init_test_items(void)
{
    setpwent();
    do {
...
static bool check_padding(char *buffer, size_t size, char pad)
{
...
static void test_one(const struct passwd *item, size_t buffer_size,
...
static void test_buffer_size(size_t buffer_size)
{
    for (int i = 0; i < test_count; ++i)
