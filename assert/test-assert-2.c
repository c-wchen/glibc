...
static void test_assert_normal(void *closure)
{
    if (closure == NULL) {
...
static void test_assert_nomalloc(void *closure)
{
    if (closure == NULL) {
        signal(SIGABRT, abort_handler);
        fail_malloc = 1;
...
static void check_posix(const char *buf, int rv, int line,
...
static void one_test(void (*func)(void *), int which_path)
{
