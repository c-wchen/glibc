...
static void fmt_ftostr(char *dest, size_t size, int precision, const char *conversion,
                       FLOAT value)
{
    char format[64];
    char *ptr_format;
...
static FLOAT default_max_valid_error(int exact, int testing_ibm128)
{
    if (testing_ibm128)
...
static int compare_ulp_data(const void *key, const void *ulp)
{
...
static FLOAT find_ulps(const char *name, const struct ulp_data *data, size_t nmemb,
...
void init_max_error(const char *name, int exact, int testing_ibm128)
{
    max_error = 0;
    real_max_error = 0;
    imag_max_error = 0;
...
static void update_stats(int ok, int testType)
{
    switch (testType) {
        case TEST_INPUT:
            ++noTests;
...
static void print_complex_function_ulps(const char *function_name, FLOAT real_ulp,
                                        FLOAT imag_ulp)
{
    if (output_ulps) {
        char fstrn[FSTR_MAX];
...
static void fpstack_test(const char *test_name)
{
...
static void print_test_start(int test_num, const char *test_name, int test_type)
{
    if (print_screen(1))
...
static void print_test_end(int test_num, const char *test_name, int test_type)
{
    if (print_screen(1))
...
void check_max_error(const char *func_name)
{
    int ok = 0;
...
/* This is a builtin test of overall max error.  */
void check_complex_max_error(const char *func_name)
{
...
static void test_single_errno(const char *test_name, int errno_value,
...
static void test_errno(const char *test_name, int errno_value, int exceptions)
{
    if (flag_test_errno) {
        int ran = 0;
...
static void check_float_internal(const char *test_name, FLOAT computed, FLOAT expected,
                                 int exceptions,
...
void check_float(const char *test_name, FLOAT computed, FLOAT expected,
                 int exceptions)
{
...
void check_complex(const char *test_name, CFLOAT computed,
                   CFLOAT expected,
                   int exception)
{
...
void check_int(const char *test_name, int computed, int expected,
               int exceptions)
{
    int ok = 0;
    int errno_value = errno;
...
void check_long(const char *test_name, long int computed, long int expected,
                int exceptions)
{
    int ok = 0;
    int errno_value = errno;
...
void check_bool(const char *test_name, int computed, int expected,
                int exceptions)
{
    int ok = 0;
    int errno_value = errno;
...
/* check that computed and expected values are equal (long int values) */
void check_longlong(const char *test_name, long long int computed,
...
void check_intmax_t (const char *test_name, intmax_t computed,
...
void check_uintmax_t (const char *test_name, uintmax_t computed,
...
int enable_test(int exceptions)
{
...
/* Do all initialization for a test run with arguments given by ARGC
   and ARGV.  */
void libm_test_init(int argc, char **argv)
{
    int remaining;
    char *ulps_file_path;
    size_t dir_len = 0;
