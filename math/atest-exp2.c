...
static void print_mpn_fp(const mp_limb_t *x, unsigned int dp, unsigned int base)
{
...
/* Compute e^x.  */
static void exp_mpn(mp1 ex, mp1 x)
{
    unsigned int n;
    mp1 xp;
    mp2 tmp;
...
/* Calculate 2^x.  */
static void exp2_mpn(mp1 ex, mp1 x)
{
    mp2 tmp;
...
static int mpn_bitsize(const mp_limb_t *SRC_PTR, mp_size_t SIZE)
{
    int i, j;
    for (i = SIZE - 1; i > 0; --i)
        if (SRC_PTR[i] != 0) {
