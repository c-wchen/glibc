...
static void lr_buffer_init(struct lr_buffer *lrb)
{
    lrb->act = 0;
    lrb->max = 56;
...
static void lr_buffer_to_token(struct lr_buffer *lrb, struct linereader *lr)
{
...
/* Adds CH to *LRB.  */
static void addc(struct lr_buffer *lrb, char ch)
{
    if (lrb->act == lrb->max) {
        lrb->max *= 2;
...
static void adds(struct lr_buffer *lrb, const unsigned char *s, size_t l)
{
...
static bool translate_unicode_codepoint(struct localedef_t *locale,
                                        const struct charmap_t *charmap,
                                        const struct repertoire_t *repertoire,
...
/* Returns true if ch is not EOF (that is, non-negative) and a valid
   UTF-8 trailing byte.  */
static bool utf8_valid_trailing(int ch)
{
...
static bool utf8_sequence_error(struct linereader *lr, uint8_t ch1, int ch2, int ch3,
                                int ch4)
{
    char buf[38];
...
static struct token *get_string(struct linereader *lr, const struct charmap_t *charmap,
                                struct localedef_t *locale, const struct repertoire_t *repertoire,
                                int verbose)
{
