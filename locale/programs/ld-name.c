...
static void name_startup(struct linereader *lr, struct localedef_t *locale,
                         int ignore_content)
{
    if (!ignore_content)
...
void name_output(struct localedef_t *locale, const struct charmap_t *charmap,
...
void name_read(struct linereader *ldfile, struct localedef_t *result,
