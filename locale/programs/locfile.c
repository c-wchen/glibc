...
static int compare_files(const char *filename1, const char *filename2, size_t size,
                         size_t blocksize)
{
    int fd1, fd2;
    int ret = -1;
...
void add_locale_empty(struct locale_file *file)
{
    record_offset(file);
}

/* Record that FILE's next element consists of SIZE bytes starting at DATA.  */
void add_locale_raw_data(struct locale_file *file, const void *data, size_t size)
{
    record_offset(file);
...
/* Record that FILE's next element is an array of N_ELEMS integers
   starting at DATA.  */
void add_locale_uint32_array(struct locale_file *file,
...
void write_locale_data(const char *output_path, int catidx, const char *category,
