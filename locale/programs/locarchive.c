...
static void *prepare_address_space(int fd, size_t total, size_t *reserved, int *xflags,
...
static void create_archive(const char *archivefname, struct locarhandle *ah)
{
    int fd;
...
static int oldlocrecentcmp(const void *a, const void *b)
{
...
static bool file_data_available_p(struct locarhandle *ah, uint32_t offset, uint32_t size)
{
...
static int compare_from_file(struct locarhandle *ah, void *p1, uint32_t offset2,
                             uint32_t size)
{
...
static struct namehashent *insert_name(struct locarhandle *ah,
...
static void add_alias(struct locarhandle *ah, const char *alias, bool replace,
...
static int          /* qsort comparator used below */ cmpcategorysize(const void *a, const void *b)
{
...
static uint32_t add_locale(struct locarhandle *ah,
...
/* Check the content of the archive for duplicates.  Add the content
   ...
   old files.  */
int add_locale_to_archive(struct locarhandle *ah, const char *name,
                          locale_data_t data, bool replace)
{
...
static int dataentcmp(const void *a, const void *b)
{
...
void show_archive_content(const char *fname, int verbose)
{
    struct locarhandle ah;
    struct locarhead *head;
    struct namehashent *namehashtab;
