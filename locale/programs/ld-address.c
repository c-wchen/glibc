...
static void address_startup(struct linereader *lr, struct localedef_t *locale,
                            int ignore_content)
{
    if (!ignore_content)
...
%s: language abbreviation `%s' not defined"),
        ...
        } else if (strcmp(iso639[cnt].ab, address->lang_ab) != 0
                   && iso639[cnt].ab[0] != '\0')
            record_error(0, 0, _("\
...
void address_output(struct localedef_t *locale, const struct charmap_t *charmap,
...
void address_read(struct linereader *ldfile, struct localedef_t *result,
