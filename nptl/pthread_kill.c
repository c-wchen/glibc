...
/* Send the signal SIGNO to the caller.  Used by abort and called where the
   ...
   exit_lock.  */
int __pthread_raise_internal(int signo)
{
...
int __pthread_kill_internal(pthread_t threadid, int signo)
{
...
/* Variant which returns ESRCH in the no-TID case, for backwards
   compatibility.  */
int
attribute_compat_text_section __pthread_kill_esrch(pthread_t threadid, int signo)
{
    if (is_internal_signal(signo)) {
        return EINVAL;
    }
