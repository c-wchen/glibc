...
int __pthread_mutex_clocklock_common(pthread_mutex_t *mutex,
                                     clockid_t clockid,
            ...
            if (oldval != 0) {
                ...
                if (e == ETIMEDOUT) {
                    ...
                    /* Delay the thread until the timeout is reached. Then return
                       ETIMEDOUT.  */
                    do
                        e = __futex_abstimed_wait64(&(unsigned int) {
                        0
                    }, 0, clockid,
                    abstime, private);
                    while (e != ETIMEDOUT);
...
int ___pthread_mutex_clocklock64(pthread_mutex_t *mutex,
                                 clockid_t clockid,
                                 const struct __timespec64 *abstime)
{
...
int ___pthread_mutex_timedlock64(pthread_mutex_t *mutex,
                                 const struct __timespec64 *abstime)
{
