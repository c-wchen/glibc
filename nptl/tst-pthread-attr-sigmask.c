...
static void check_sigmask(const char *context, signals expected_mask,
...
static void check_current_sigmask(const char *context, signals expected_mask)
{
...
/* Thread start routine which checks the current thread signal mask
   against CLOSURE.  */
static void *check_sigmask_thread_function(void *closure)
{
...
static int check_sigmask_thread_function_c11(void *closure)
{
...
static void check_posix_thread(pthread_attr_t *attr, signals expected_mask)
{
...
/* Launch a C11 thread and check that it has the expected signal
   mask.  */
static void check_c11_thread(signals expected_mask)
{
    thrd_t thr;
...
static int do_test(void)
{
    ...
    check_sigmask("pthread_attr_getsigmask_np", (signals) {
        [SIGUSR2] = true,
    },
    &set);
    ...
    sigaddset(&set, SIGHUP);
    TEST_COMPARE(pthread_attr_getsigmask_np(&attr, &set), 0);
    check_sigmask("default attribute", (signals) {
        [SIGHUP] = true,
    }, &set);
    xpthread_attr_destroy(&attr);
