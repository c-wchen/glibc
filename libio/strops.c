...
void _IO_str_init_static_internal(_IO_strfile *sf, char *ptr, size_t size,
                                  char *pstart)
{
    FILE *fp = &sf->_sbf._f;
    char *end;
...
static void _IO_str_switch_to_get_mode(FILE *fp)
{
    if (_IO_in_backup(fp)) {
...
off64_t _IO_str_seekoff(FILE *fp, off64_t offset, int dir, int mode)
{
    off64_t new_pos;
