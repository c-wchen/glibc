...
static void log_method(FILE *fp, const char *name)
{
    if (test_verbose > 0) {
...
static void method_finish(FILE *fp, int dummy)
{
...
static int method_overflow(FILE *fp, int ch)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static int method_underflow(FILE *fp)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static int method_uflow(FILE *fp)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static int method_pbackfail(FILE *fp, int ch)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static size_t method_xsputn(FILE *fp, const void *data, size_t n)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static size_t method_xsgetn(FILE *fp, void *data, size_t n)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static off64_t method_seekoff(FILE *fp, off64_t offset, int dir, int mode)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static off64_t method_seekpos(FILE *fp, off64_t offset, int mode)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static FILE *method_setbuf(FILE *fp, char *buffer, ssize_t length)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static int method_sync(FILE *fp)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static int method_doallocate(FILE *fp)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static ssize_t method_read(FILE *fp, void *data, ssize_t length)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static ssize_t method_write(FILE *fp, const void *data, ssize_t length)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static off64_t method_seek(FILE *fp, off64_t offset, int mode)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static int method_close(FILE *fp)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static int method_stat(FILE *fp, void *buffer)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static int method_showmanyc(FILE *fp)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static void method_imbue(FILE *fp, void *locale)
{
    log_method(fp, __func__);
    TEST_VERIFY(fp == shared->fp);
    ++shared->calls;
...
static void without_compatibility_fprintf(void *closure)
{
...
static void without_compatibility_fputc(void *closure)
{
    /* This call should abort.  */
    fputc(' ', shared->fp);
    _exit(1);
...
static void without_compatibility_fgetc(void *closure)
{
    /* This call should abort.  */
    fgetc(shared->fp);
    _exit(1);
...
static void without_compatibility_fflush(void *closure)
{
    /* This call should abort.  */
    fflush(shared->fp);
    _exit(1);
...
static void check_for_termination(const char *name, void (*callback)(void *))
{
...
static void with_compatibility_fprintf(void *closure)
{
...
static void with_compatibility_fputc(void *closure)
{
    shared->return_value = '@';
...
static void with_compatibility_fgetc(void *closure)
{
...
static void with_compatibility_fflush(void *closure)
{
...
static void check_call(const char *name, void (*callback)(void *),
                       bool initially_disabled)
{
...
static int run_tests(bool initially_disabled)
{
