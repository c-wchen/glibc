...
off64_t _IO_default_seekpos(FILE *fp, off64_t pos, int mode)
{
    return _IO_SEEKOFF(fp, pos, 0, mode);
}

int _IO_default_doallocate(FILE *fp)
{
    char *buf;
...
int _IO_sputbackc(FILE *fp, int c)
{
    int result;
...
void _IO_init_marker(struct _IO_marker *marker, FILE *fp)
{
    marker->_sbuf = fp;
    if (_IO_in_put_mode(fp)) {
...
int _IO_seekmark(FILE *fp, struct _IO_marker *mark, int delta)
{
    if (mark->_sbuf != fp) {
        return EOF;
    }
...
int _IO_default_stat(FILE *fp, void *st)
{
    return EOF;
}

ssize_t _IO_default_read(FILE *fp, void *data, ssize_t n)
{
    return -1;
}

ssize_t _IO_default_write(FILE *fp, const void *data, ssize_t n)
{
    return 0;
}

int _IO_default_showmanyc(FILE *fp)
{
    return -1;
}

void _IO_default_imbue(FILE *fp, void *locale)
{
}
