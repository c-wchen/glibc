...
static int do_ftruncate_test(const char *filename)
{
    FILE *fp = NULL;
    int fd;
    int ret = 0;
...
static int do_rewind_test(const char *filename)
{
    int ret = 0;
    struct test {
        const char *mode;
...
/* Test that the value of ftell is not cached when the stream handle is not
   active.  */
static int do_ftell_test(const char *filename)
{
    int ret = 0;
    struct test {
        const char *mode;
...
static int do_write_test(const char *filename)
{
    FILE *fp = NULL;
    int fd;
    int ret = 0;
...
static int do_append_test(const char *filename)
{
    FILE *fp = NULL;
    int ret = 0;
    int fd;
