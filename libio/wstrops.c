...
void _IO_wstr_init_static(FILE *fp, wchar_t *ptr, size_t size,
                          wchar_t *pstart)
{
    wchar_t *end;
...
wint_t _IO_wstr_overflow(FILE *fp, wint_t c)
{
...
wint_t _IO_wstr_underflow(FILE *fp)
{
    if (fp->_wide_data->_IO_write_ptr > fp->_wide_data->_IO_read_end) {
        fp->_wide_data->_IO_read_end = fp->_wide_data->_IO_write_ptr;
    }
...
ssize_t _IO_wstr_count(FILE *fp)
{
...
static int enlarge_userbuf(FILE *fp, off64_t offset, int reading)
{
...
static void _IO_wstr_switch_to_get_mode(FILE *fp)
{
    if (_IO_in_backup(fp)) {
...
off64_t _IO_wstr_seekoff(FILE *fp, off64_t offset, int dir, int mode)
{
    off64_t new_pos;
...
wint_t _IO_wstr_pbackfail(FILE *fp, wint_t c)
{
