...
ssize_t _IO_least_wmarker(FILE *fp, wchar_t *end_p)
{
...
wint_t _IO_sputbackwc(FILE *fp, wint_t c)
{
    wint_t result;
...
unsigned _IO_adjust_wcolumn(unsigned start, const wchar_t *line, int count)
{
...
void _IO_init_wmarker(struct _IO_marker *marker, FILE *fp)
{
    marker->_sbuf = fp;
    if (_IO_in_put_mode(fp)) {
...
int _IO_wmarker_delta(struct _IO_marker *mark)
{
    int cur_pos;
    if (mark->_sbuf == NULL) {
        return BAD_DELTA;
...
int _IO_seekwmark(FILE *fp, struct _IO_marker *mark, int delta)
{
    if (mark->_sbuf != fp) {
        return EOF;
    }
...
void _IO_unsave_wmarkers(FILE *fp)
{
