...
int _IO_file_underflow_mmap(FILE *fp)
{
    if (fp->_IO_read_ptr < fp->_IO_read_end) {
        return *(unsigned char *) fp->_IO_read_ptr;
    }
...
static void decide_maybe_mmap(FILE *fp)
{
...
static off64_t do_ftell(FILE *fp)
{
...
off64_t _IO_new_file_seekoff(FILE *fp, off64_t offset, int dir, int mode)
{
    off64_t result;
...
off64_t _IO_file_seekoff_maybe_mmap(FILE *fp, off64_t offset, int dir,
                                    int mode)
{
...
size_t _IO_file_xsgetn_maybe_mmap(FILE *fp, void *data, size_t n)
{
