...
static size_t socket_address_length(int family)
{
    switch (family) {
        case AF_INET:
...
static void format_ai_flags_1(FILE *out, struct addrinfo *ai, int flag, const char *name,
                              int *flags_printed)
{
...
static void format_ai_flags(FILE *out, struct addrinfo *ai)
{
    if (ai == NULL) {
        return;
    }
...
static void format_ai_canonname(FILE *out, struct addrinfo *ai)
{
    if (ai == NULL) {
        return;
    }
...
static void format_ai_one(FILE *out, struct addrinfo *ai)
{
    {
        char type_buf[32];
        const char *type_str;
...
static void format_ai_family(FILE *out, struct addrinfo *ai, int family)
{
    while (ai) {
...
char *support_format_addrinfo(struct addrinfo *ai, int ret)
{
