...
unsigned int support_readdir_inode_width(enum support_readdir_op op)
{
    switch (op) {
        ...
        case SUPPORT_READDIR64_COMPAT:
        case SUPPORT_READDIR64_R_COMPAT:
            return sizeof((struct __old_dirent64) {
                0,
            } .d_ino) * 8;
    }
    FAIL_EXIT1("invalid support_readdir_op constant: %d", op);
...
static bool copy_dirent(struct support_dirent *dst, struct dirent *src)
{
    if (src == NULL) {
        return false;
    }
...
static bool copy_dirent64(struct support_dirent *dst, struct dirent64 *src)
{
    if (src == NULL) {
        return false;
    }
...
static bool copy_old_dirent64(struct support_dirent *dst, struct __old_dirent64 *src)
{
    if (src == NULL) {
        return false;
    }
