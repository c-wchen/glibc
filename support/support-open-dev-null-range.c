...
static void increase_nofile(void)
{
    struct rlimit rl;
...
static int open_dev_null(int flags, mode_t mode)
{
...
static void range_init(struct range_list *r)
{
    r->total = 8;
    r->used = 0;
...
static void range_add(struct range_list *r, int lowfd, size_t len)
{
    if (r->used == r->total) {
        r->total *= 2;
...
static void range_close(struct range_list *r)
{
