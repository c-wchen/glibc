...
/* Write one byte at *P to FD and advance *P.  Do nothing if *P is
   '\0'.  */
static void transfer(const unsigned char **p, int fd)
{
    if (**p != '\0') {
...
static const char *write_mode_to_str(enum write_mode mode)
{
    switch (mode) {
        case out_first:
            return "out_first";
...
_Noreturn static void test_common(const struct test *test)
{
    bool mode_ok = false;
...
_Noreturn static void handle_restart(char *out, char *err, const char *write_mode,
...
_Noreturn static void callback(void *closure)
{
...
/* Check that the specific stream from the captured subprocess matches
   expectations.  */
static void check_stream(const char *what, const struct xmemstream *stream,
                         const char *expected)
{
...
static struct support_capture_subprocess do_subprocess(struct test *test)
{
    return support_capture_subprocess(callback, test);
}

static struct support_capture_subprocess do_subprogram(const struct test *test)
{
...
static int do_multiple_tests(enum test_type type)
{
