...
static void fill0(char *target, const char *element, size_t element_size,
                  size_t count)
{
    while (count > 0) {
...
static void fill(char *target, const char *element, size_t element_size,
                 size_t count)
{
...
/* Use malloc instead of mmap for small allocations and unusual size
   combinations.  */
static struct support_blob_repeat allocate_malloc(size_t total_size, const void *element, size_t element_size,
        size_t count)
{
...
/* Return the least common multiple of PAGE_SIZE and ELEMENT_SIZE,
   ...
   two.  */
static size_t minimum_stride_size(size_t page_size, size_t element_size)
{
...
static struct support_blob_repeat allocate_big(size_t total_size, const void *element, size_t element_size,
...
void support_blob_repeat_free(struct support_blob_repeat *blob)
{
    if (blob->size > 0) {
        int saved_errno = errno;
        if (blob->use_malloc) {
