...
void maybe_xunlink(const char *path)
{
    int rv = unlink(path);
...
void maybe_xmkdir(const char *path, mode_t mode)
{
    struct stat st;
...
static char *concat_words(char **words, int num_words)
{
    int len = 0;
    int i;
    char *rv, *p;
...
static void trymount(const char *src, const char *dest)
{
...
static void devmount(const char *new_root_path, const char *which)
{
    int fd;
...
/* Returns true if the string "looks like" an environment variable
   being set.  */
static int is_env_setting(const char *a)
{
    int count_name = 0;
...
static int tokenize(char *the_line, char **the_words, int nwords)
{
    int rv = 0;
...
static void r_setup(char *path, path_buf *pb)
{
...
static void r_append(const char *path, path_buf *pb)
{
...
static void recursive_remove(char *path)
{
...
static void copy_one_file(const char *sname, const char *dname)
{
    int sfd, dfd;
    struct stat st;
...
static int need_sync(char *ap, char *bp, struct stat *a, struct stat *b)
{
...
static void rsync_1(path_buf *src, path_buf *dest, int and_delete, int force_copies)
{
    DIR *dir;
    struct dirent *de;
    struct stat s, d;
...
static void rsync(char *src, char *dest, int and_delete, int force_copies)
{
    r_setup(src, &spath);
    r_setup(dest, &dpath);
...
void check_for_unshare_hints(int require_pidns)
{
    static struct {
        const char *path;
