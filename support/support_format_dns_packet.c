...
static inline bool extract_16(struct in_buffer *in, unsigned short *value)
{
    if (in->size < 2) {
        return false;
    }
...
static inline bool extract_32(struct in_buffer *in, unsigned *value)
{
    if (in->size < 4) {
        return false;
    }
...
static inline bool extract_bytes(struct in_buffer *in, size_t length, struct in_buffer *value)
{
    if (in->size < length) {
        return false;
    }
    *value = (struct in_buffer) {
        in->data, length
    };
    in->data += length;
    in->size -= length;
...
static bool extract_name(struct in_buffer full, struct in_buffer *in, struct dname *value)
{
...
static void extract_name_data(struct in_buffer full, struct in_buffer *rdata,
