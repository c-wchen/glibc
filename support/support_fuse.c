...
void support_fuse_filter_forget(struct support_fuse *f, bool filter)
{
    f->filter_forget = filter;
}

void *support_fuse_cast_internal(struct fuse_in_header *p, uint32_t expected)
{
...
void *support_fuse_cast_name_internal(struct fuse_in_header *p, uint32_t expected,
...
bool support_fuse_handle_directory(struct support_fuse *f)
{
...
bool support_fuse_handle_mountpoint(struct support_fuse *f)
{
    TEST_VERIFY(f->inh != NULL);
    /* 1 is the root node.  */
...
static char *support_fuse_mkdir(const char *prefix)
{
...
const char *support_fuse_mountpoint(struct support_fuse *f)
{
    return f->mountpoint;
}

void support_fuse_no_reply(struct support_fuse *f)
{
    TEST_VERIFY(f->inh != NULL);
...
char *support_fuse_opcode(uint32_t op)
{
    const char *result;
    switch (op) {
...
void support_fuse_reply_empty(struct support_fuse *f)
{
    support_fuse_reply_error_1(f, 0);
}

static void support_fuse_reply_error_1(struct support_fuse *f, uint32_t error)
{
    TEST_VERIFY_EXIT(f->inh != NULL);
    struct fuse_out_header outh = {
        .len = sizeof(outh),
...
void support_fuse_reply_prepared(struct support_fuse *f)
{
...
static void *support_fuse_thread_wrapper(void *closure)
{
    struct fuse_thread_wrapper_args args
...
void support_fuse_unmount(struct support_fuse *f)
{
