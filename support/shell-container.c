...
static int true_func(char **argv)
{
    return 0;
}
...
static int echo_func(char **argv)
{
    int i;
...
/* Emulate the "/bin/cp" command.  Options are ignored.  Only copies
   ...
   are not supported.  */
static int copy_func(char **argv)
{
    char *sname = argv[0];
    char *dname = argv[1];
    int sfd = -1, dfd = -1;
...
static int exit_func(char **argv)
{
    int exit_val = 0;
...
static int kill_func(char **argv)
{
    int signum = SIGTERM;
    int i;
...
/* Emulate the "/bin/sleep" command.  No suffix support.  Options are
   ignored.  */
static int sleep_func(char **argv)
{
    if (argv[0] == NULL) {
...
/* Run one tokenized command.  argv[0] is the command.  argv is
   NULL-terminated.  */
static void run_command_array(char **argv)
{
    int i, j;
    pid_t pid;
    int status;
...
static void run_command_string(const char *cmdline, const char **iargs)
{
...
/* Run a script by reading lines and passing them to the above
   function.  */
static void run_script(const char *filename, const char **args)
{
