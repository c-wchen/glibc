...
static struct compressed_name *allocate_compressed_name(const unsigned char *encoded, unsigned int offset)
{
...
static int compare_compressed_name(const void *left, const void *right)
{
...
void resolv_response_init(struct resolv_response_builder *b,
...
void resolv_response_section(struct resolv_response_builder *b, ns_sect section)
{
    if (b->offset == 0) {
...
static inline void response_add_byte(struct resolv_response_builder *b, unsigned char ch)
{
...
static void response_add_16(struct resolv_response_builder *b, uint16_t val)
{
...
static void response_count_increment(struct resolv_response_builder *b)
{
...
void resolv_response_add_question(struct resolv_response_builder *b,
...
void resolv_response_add_name(struct resolv_response_builder *b,
...
void resolv_response_open_record(struct resolv_response_builder *b,
                                 const char *name,
...
void resolv_response_close_record(struct resolv_response_builder *b)
{
...
void resolv_response_add_data(struct resolv_response_builder *b,
...
void resolv_response_drop(struct resolv_response_builder *b)
{
    b->drop = true;
}

void resolv_response_close(struct resolv_response_builder *b)
{
    b->close = true;
}

void resolv_response_truncate_data(struct resolv_response_builder *b, size_t count)
{
    if (count > 65535)
...
size_t resolv_response_length(const struct resolv_response_builder *b)
{
    return b->offset;
}

unsigned char *resolv_response_buffer(const struct resolv_response_builder *b)
{
...
void resolv_response_builder_free(struct resolv_response_builder *b)
{
...
static void parse_query(struct query_info *info,
...
/* Start a server thread for the specified SERVER_INDEX, implemented
   by CALLBACK.  */
static pthread_t start_server_thread(struct resolv_test *obj, int server_index,
...
/* Process one UDP query.  Return false if a termination requested has
   been detected.  */
static bool server_thread_udp_process_one(struct resolv_test *obj, int server_index)
{
...
void resolv_response_send_udp(const struct resolv_response_context *ctx,
                              struct resolv_response_builder *b)
{
...
/* UDP thread_callback function.  Variant for one thread per
   server.  */
static void server_thread_udp(struct resolv_test *obj, int server_index)
{
...
/* Single-threaded UDP processing function, for the single_thread_udp
   case.  */
static void *server_thread_udp_single(void *closure)
{
...
/* Start the single UDP handler thread (for the single_thread_udp
   case).  */
static void start_server_thread_udp_single(struct resolv_test *obj)
{
...
static bool read_fully(int fd, void *buf, size_t len, bool eof_ok)
{
...
static void writev_fully(int fd, struct iovec *buffers, size_t count)
{
    while (count > 0) {
...
/* Thread callback for handling a single established TCP connection to
   a client.  */
static void *server_thread_tcp_client(void *arg)
{
...
static void server_thread_tcp(struct resolv_test *obj, int server_index)
{
    while (true) {
...
static void make_server_sockets(struct resolv_test_server *server)
{
    while (true) {
...
/* Like make_server_sockets, but the caller supplies the address to
   use.  */
static void make_server_sockets_for_address(struct resolv_test_server *server,
...
static void resolv_redirect_once(void)
{
    /* Only use nss_dns.  */
...
static void set_search_path(struct resolv_redirect_config config)
{
...
void resolv_test_end(struct resolv_test *obj)
{
    res_close();
