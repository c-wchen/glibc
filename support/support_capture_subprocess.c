...
static void transfer(const char *what, struct pollfd *pfd, struct xmemstream *stream)
{
    if (pfd->revents != 0) {
        char buf[1024];
...
static void support_capture_poll(struct support_capture_subprocess *result,
...
/* Copies the executable into a restricted directory, so that we can
   ...
   executable.  */
static void copy_and_spawn_sgid(const char *child_id, gid_t gid)
{
...
/* Returns true if a group with NAME has been found, and writes its
   GID to *TARGET.  */
static bool find_sgid_group(gid_t *target, const char *name)
{
