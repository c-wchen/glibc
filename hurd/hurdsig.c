...

static void default_sigaction(struct sigaction actions[NSIG])
{
    int signo;
...
static int write_corefile(int signo, const struct hurd_signal_detail *detail)
{
    error_t err;
...
static void abort_thread(struct hurd_sigstate *ss, struct machine_thread_all_state *state,
                         void (*reply)(void))
{
    assert(ss->thread != MACH_PORT_NULL);
...
static mach_port_t *interrupted_reply_port_location(thread_t thread,
...
mach_port_t _hurdsig_abort_rpcs(struct hurd_sigstate *ss, int signo, int sigthread,
...
static void abort_all_rpcs(int signo, struct machine_thread_all_state *state, int live)
{
...
/* Wake up any sigsuspend or pselect call that is blocking SS->thread.  SS must
   be locked.  */
static void wake_sigsuspend(struct hurd_sigstate *ss)
{
    error_t err;
...
static sigset_t pending_signals(struct hurd_sigstate *ss)
{
...
static int post_pending(struct hurd_sigstate *ss, sigset_t pending, void (*reply)(void))
{
    int signo;
...
static int post_all_pending_signals(void (*reply)(void))
{
    struct hurd_sigstate *ss;
    sigset_t pending = 0;
...
static error_t signal_allowed(int signo, mach_port_t refport)
{
...
kern_return_t _S_msg_sig_post(mach_port_t me,
...
kern_return_t _S_msg_sig_post_untraced(mach_port_t me,
                                       mach_port_t reply_port,
                                       mach_msg_type_name_t reply_port_type,
                                       int signo, natural_t sigcode,
                                       mach_port_t refport)
...
static void reauth_proc(mach_port_t new)
{
    error_t err;
...
const char *_hurdsig_getenv(const char *variable)
{
