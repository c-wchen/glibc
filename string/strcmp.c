...
static inline int final_cmp(const op_t w1, const op_t w2)
{
...
static inline int strcmp_aligned_loop(const op_t *x1, const op_t *x2, op_t w1)
{
    op_t w2 = *x2++;
...
/* Unaligned loop: align the first partial of P2, with 0xff for the rest of
   ...
   final comparison.  */
static inline int strcmp_unaligned_loop(const op_t *x1, const op_t *x2, op_t w1, uintptr_t ofs)
{
    op_t w2a = *x2++;
