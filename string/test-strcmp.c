...
int simple_wcscmp(const wchar_t *s1, const wchar_t *s2)
{
    wchar_t c1, c2;
    do {
        c1 = *s1++;
    ...
    for (size_t n = 0; n < ITERATIONS; n++) {
        ...
        FOR_EACH_IMPL(impl, 1) {
            ...
            if ((r == 0 && result)
                ...
                || (r > 0 && result <= 0)) {
                error(0, 0,
                      "Iteration %zd - wrong result in function %s (align in bytes: %zd, align in bytes: %zd, len1:  %zd, len2: %zd, pos: %zd) %d != %d, p1 %p p2 %p",
