...
static __always_inline char *write_byte_from_word(op_t *dest, op_t word)
{
    char *d = (char *) dest;
...
static __always_inline char *stpcpy_aligned_loop(op_t *restrict dst, const op_t *restrict src)
{
    op_t word;
    while (1) {
        word = *src++;
...
static __always_inline char *stpcpy_unaligned_loop(op_t *restrict dst, const op_t *restrict src,
        uintptr_t ofs)
{
    op_t w2a = *src++;
