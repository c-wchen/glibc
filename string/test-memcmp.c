...
int SIMPLE_MEMCMP(const wchar_t *s1, const wchar_t *s2, size_t n)
{
    int ret = 0;
    /* Warning!
    ...
    */
    while (n-- && (ret = *s1 < *s2 ? -1 : *s1 == *s2 ? 0 : 1) == 0) {
        s1++;
        s2++;
    }
    return ret;
...
static void do_one_test(impl_t *impl, const CHAR *s1, const CHAR *s2, size_t len,
                        int exp_result)
{
...
static void do_test(size_t align1, size_t align2, size_t len, int exp_result)
{
    size_t i;
    CHAR *s1, *s2;
