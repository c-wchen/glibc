...
static void print_buffer(const char *s, size_t n)
{
    size_t i;
    printf("{");
    for (i = 0; i < n; ++i) {
...
static int do_test_write_append(const char *mode)
{
...
static int do_test_write_append_without_null(const char *mode)
{
...
static int do_test_read_append(void)
{
    char buf[32] = "testing buffer";
...
static int do_test_read_seek_neg(const char *mode, const char *expected)
{
...
static int do_test_read_seek_negative(void)
{
    int ret = 0;
...
static int do_test_write_append_2(const char *str)
{
    char buf[10];
