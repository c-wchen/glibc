...
/* Creates two large, random strings used for truncation testing.
   ...
   free.  */
static void make_random_string(char **mbs, wchar_t **wide, size_t **length)
{
    struct str str;
    str_init(&str);
    struct wstr wstr;
...
static void test_mbs_result(void)
{
    char buf[200];
...
static void test_wide_result(void)
{
    enum { size = 20 };
    wchar_t buf[20];
...
static void test_mbs_long(const char *mbs, const wchar_t *wide, const size_t *length)
{
    char buf[4000];
...
static void test_wide_long(const char *mbs, const wchar_t *wide, const size_t *length)
{
