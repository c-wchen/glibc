...
static ssize_t io_write(void *vcookie, const char *buf, size_t size)
{
    VALIDATE_COOKIE();
    FAIL_EXIT1("io_write called");
}

static int io_seek(void *vcookie, off64_t *position, int whence)
{
    VALIDATE_COOKIE();
    FAIL_EXIT1("io_seek called");
}

static int io_clean(void *vcookie)
{
    struct Cookie *cookie = (struct Cookie *) vcookie;
...
FILE *io_open(const char *buffer, int buflen, const char *mode, void **vcookie)
{
    FILE *f;
