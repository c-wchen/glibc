...
static char hack_digit(struct hack_digit_param *p)
{
    mp_limb_t hi;
...
static void __printf_fp_buffer_1(struct __printf_buffer *buf, locale_t loc,
...
void __printf_buffer_flush_fp(struct __printf_buffer_fp *buf)
{
...
void __printf_fp_l_buffer(struct __printf_buffer *buf, locale_t loc,
                          const struct printf_info *info,
                          const void *const *args)
{
...
void __printf_buffer_flush_fp_to_wide(struct __printf_buffer_fp_to_wide *buf)
{
    /* No need to update buf->base.written; the actual count is
       maintained in buf->next->written.  */
    for (char *p = buf->untranslated; p < buf->base.write_ptr; ++p) {
...
void __wprintf_fp_l_buffer(struct __wprintf_buffer *buf, locale_t loc,
                           const struct printf_info *info,
                           const void *const *args)
{
...
int ___printf_fp(FILE *fp, const struct printf_info *info,
                 const void *const *args)
{
    if (info->wide) {
