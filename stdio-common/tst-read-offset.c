...
static FILE *open_test_stream(enum test_open_case c)
{
    FILE *fp;
    switch (c) {
...
static int base_loc_to_bytes(enum test_base_loc loc, int offset)
{
    switch (loc) {
        case base_loc_start:
            return offset;
...
static int clear_offset(enum clear_ungetc_case c, int num_ungetc)
{
    switch (c) {
        case clear_fseekm1:
        case clear_fseekom1:
...
static int do_clear_ungetc(FILE *fp, enum clear_ungetc_case c, int num_ungetc)
{
    int ret;
...
static bool clear_valid(enum test_open_case c, enum clear_ungetc_case cl)
{
    switch (c) {
        case test_open_fmemopen:
...
static bool clear_closes_file(enum clear_ungetc_case cl)
{
    switch (cl) {
        case clear_fclose:
            return true;
...
static void clear_getpos_before(FILE *fp, enum clear_ungetc_case c)
{
    switch (c) {
        case clear_fsetposu:
...
static void clear_getpos_after(FILE *fp, enum clear_ungetc_case c)
{
    switch (c) {
        case clear_fsetpos:
            TEST_COMPARE(fgetpos(fp, &pos), 0);
...
static bool valid_fd_offset(enum test_open_case c, enum clear_ungetc_case cl)
{
    switch (c) {
        case test_open_fmemopen:
...
static bool verify_valid(enum test_open_case c, enum clear_ungetc_case cl,
                         enum verify_case v)
{
    switch (v) {
        case verify_fd:
...
static bool verify_uses_fd(enum verify_case v)
{
    switch (v) {
        case verify_fd:
            return true;
...
static int read_to_test_loc(FILE *fp, enum test_base_loc loc, int offset)
{
...
static void test_one_case(enum test_open_case c, enum test_base_loc loc, int offset,
