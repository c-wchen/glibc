...
static void end_thread(pthread_t *ptid)
{
    if (*ptid) {
        pthread_cancel(*ptid);
        xpthread_join(*ptid);
...
static void *writer_thread_proc(void *closure)
{
...
static void *reader_thread_proc(void *closure)
{
    ThreadData *td = (ThreadData *) closure;
    int fd;
    ssize_t rn;
...
static void start_writer_thread(int fd)
{
    debug;
    thread_data.fd = fd;
...
static void start_writer_thread_n(const char *fname)
{
    debug;
    thread_data.fd = -1;
...
static void start_reader_thread(int fd)
{
    debug;
    thread_data.fd = fd;
    thread_data.fname = NULL;
...
static void start_reader_thread_n(const char *fname)
{
    debug;
    thread_data.fd = -1;
    thread_data.fname = fname;
...
static void end_reader_thread(void)
{
    debug;
...
static const char *prepare_test_file(enum test_source_case f, enum test_stream_case s)
{
    debug;
...
static void unprepare_test_file(FILE *fp,
...
static FILE *open_test_stream(enum test_source_case f, enum test_stream_case s)
{
    int fd;
    FILE *fp;
    const char *fname;
...
static void test_put_string(FILE *fp, const char *s, int count)
{
    while (*s && count--) {
        fputc(*s++, fp);
...
int verify_fully_buffered(FILE *fp,
                          enum test_source_case f,
                          enum test_stream_case s,
...
int verify_line_buffered(FILE *fp,
                         enum test_source_case f,
                         enum test_stream_case s,
                         enum test_config_case c)
{
...
static int do_setvbuf(FILE *fp, void *buf, int flags, int size,
                      enum test_stream_case s)
{
