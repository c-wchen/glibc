...
static inline CHAR_T *char_buffer_start(const struct char_buffer *buffer)
{
...
/* Returns the number of CHAR_T objects in the buffer.  Only valid if
   char_buffer_error (BUFFER) is false.  */
static inline size_t char_buffer_size(const struct char_buffer *buffer)
{
...
/* Reinitializes BUFFER->current and BUFFER->end to cover the entire
   scratch buffer.  */
static inline void char_buffer_rewind(struct char_buffer *buffer)
{
...
/* Returns true if a previous call to char_buffer_add (BUFFER, CH)
   failed.  */
static inline bool char_buffer_error(const struct char_buffer *buffer)
{
    return __glibc_unlikely(buffer->current == NULL);
}

/* Slow path for char_buffer_add.  */
static void char_buffer_add_slow(struct char_buffer *buffer, CHAR_T ch)
{
...
static inline void char_buffer_add(struct char_buffer *buffer, CHAR_T ch)
