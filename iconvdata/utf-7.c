...
static uint32_t shift_character(enum variant const var)
{
    if (var == UTF7) {
        return '+';
...
static bool between(uint32_t const ch,
...
static bool isdirect(uint32_t ch, enum variant var)
{
    if (var == UTF7)
...
static bool isxdirect(uint32_t ch, enum variant var)
{
    if (isdirect(ch, var)) {
        return true;
    }
...
static bool needs_explicit_shift(uint32_t ch)
{
    return (between(ch, 'A', 'Z')
            || between(ch, 'a', 'z')
            || between(ch, '/', '9') || ch == '+' || ch == '-');
...
static unsigned char base64(unsigned int i, enum variant var)
{
    if (i < 26) {
        return i + 'A';
    } else if (i < 52) {
