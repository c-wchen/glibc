...
static bool is_trusted_path_normalize(const char *path, size_t len)
{
    if (len == 0) {
        return false;
    }
...
static size_t is_dst(const char *input, const char *ref)
{
    bool is_curly = false;
...
size_t _dl_dst_count(const char *input)
{
    size_t cnt = 0;
...
/* Process INPUT for DSTs and store in RESULT using the information
   ...
   list delimiters.  */
char *_dl_dst_substitute(struct link_map *l, const char *input, char *result)
{
...
/* Return a malloc allocated copy of INPUT with all recognized DSTs
   ...
   is returned.  */
static char *expand_dynamic_string_token(struct link_map *l, const char *input)
{
...
static void add_name_to_object(struct link_map *l, const char *name)
{
...
static bool decompose_rpath(struct r_search_path_struct *sps,
...
static bool cache_rpath(struct link_map *l,
...
static void _dl_notify_new_object(int mode, Lmid_t nsid, struct link_map *l)
{
...

/* Print search path.  */
static void print_search_path(struct r_search_path_elem **list,
...
static int open_verify(const char *name, int fd,
...
static int open_path(const char *name, size_t namelen, int mode,
...
static void add_path(struct add_path_state *p, const struct r_search_path_struct *sps,
                     unsigned int flags)
{
    if (sps->dirs != (void *) -1) {
        struct r_search_path_elem **dirs = sps->dirs;
