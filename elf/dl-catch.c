...
static struct rtld_catch *get_catch(void)
{
#if IS_IN (rtld)
...
static void set_catch(struct rtld_catch *catch)
{
#if IS_IN (rtld)
    if (!__rtld_tls_init_tp_called) {
...
void _dl_signal_exception(int errcode, struct dl_exception *exception,
                          const char *occasion)
{
...
void _dl_signal_cexception(int errcode, struct dl_exception *exception,
                           const char *occasion)
{
...
void _dl_signal_cerror(int errcode, const char *objname, const char *occasion,
                       const char *errstring)
{
    if (__builtin_expect(GLRO(dl_debug_mask)
                         & ~(DL_DEBUG_STATISTICS), 0))
...
void _dl_receive_error(receiver_fct fct, void (*operate)(void *), void *args)
{
