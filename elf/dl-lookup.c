...
static void enter_unique_sym(struct unique_sym *table, size_t size,
...
static void mark_nodelete(struct link_map *map, int flags)
{
...
/* Return true if MAP is marked as NODELETE according to the lookup
   mode in FLAGS> */
static bool is_nodelete(struct link_map *map, int flags)
{
...
static void do_lookup_unique(const char *undef_name, unsigned int new_hash,
...
static int
__attribute_noinline__ do_lookup_x(const char *undef_name, unsigned int new_hash,
...
static int add_dependency(struct link_map *undef_map, struct link_map *map, int flags)
{
