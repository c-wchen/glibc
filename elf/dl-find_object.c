...
/* Fallback implementation of _dl_find_object.  It uses a linear
   ...
   _dl_find_object.  */
static int _dl_find_object_slow(void *pc, struct dl_find_object *result)
{
...
/* Returns the number of actually used elements in all segments
   starting at SEG.  */
static inline size_t _dlfo_mappings_segment_count_used(struct dlfo_mappings_segment *seg)
{
    size_t count = 0;
...
/* Compute the total number of available allocated segments linked
   from SEG.  */
static inline size_t _dlfo_mappings_segment_count_allocated(struct dlfo_mappings_segment *seg)
{
    size_t count = 0;
...
/* Allocate an empty segment.  This used for the first ever
   allocation.  */
static struct dlfo_mappings_segment *_dlfo_mappings_segment_allocate_unpadded(size_t size)
{
...
/* Allocate an empty segment that is at least SIZE large.  PREVIOUS
   ...
   NULL.  */
static struct dlfo_mappings_segment *_dlfo_mappings_segment_allocate(size_t size,
        struct dlfo_mappings_segment *previous)
{
...
static inline uint64_t _dlfo_read_start_version(void)
{
...
static inline uint64_t _dlfo_read_version_locked(void)
{
...
static inline void _dlfo_mappings_begin_update(void)
{
...
static inline void _dlfo_mappings_end_update(void)
{
    /* The fence synchronizes with loads in _dlfo_read_start_version
       (also called from _dlfo_read_success).  */
    atomic_thread_fence_release();
...
/* Return true if the read was successful, given the start
   version.  */
static inline bool _dlfo_read_success(uint64_t start_version)
{
...
/* Returns the active segment identified by the specified start
   version.  */
static struct dlfo_mappings_segment *_dlfo_mappings_active_segment(uint64_t start_version)
{
...
static inline struct dl_find_object_internal *_dlfo_lookup(uintptr_t pc, struct dl_find_object_internal *first1,
        size_t size)
{
...
void _dlfo_sort_mappings(struct dl_find_object_internal *objects, size_t size)
{
    if (size < 2) {
        return;
    }
...
static void _dl_find_object_link_map_sort(struct link_map **loaded, size_t size)
{
...
static inline size_t _dlfo_update_init_seg(struct dlfo_mappings_segment *seg,
...
static bool _dl_find_object_update_1(struct link_map **loaded, size_t count)
{
...
bool _dl_find_object_update(struct link_map *new_map)
{
...
void _dl_find_object_dlclose(struct link_map *map)
{
