...
/* Computes the expected _dl_find_object result directly from the
   map.  */
static void from_map(struct link_map *l, struct dl_find_object *expected)
{
...
static void check(void *address, struct dl_find_object *expected, int line)
{
...
static void *exit_thread(void *ignored)
{
    usleep(300 * 1000);
...
static void *verify_thread(void *closure)
{
...
static void start_verify(int number, struct verify_data *data)
{
