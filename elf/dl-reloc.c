...
void
__attribute_noinline__ _dl_allocate_static_tls(struct link_map *map)
{
    if (map->l_tls_offset == FORCED_DYNAMIC_TLS_OFFSET
...
void _dl_nothread_init_static_tls(struct link_map *map)
{
#if TLS_TCB_AT_TP
...
static __always_inline lookup_t resolve_map(lookup_t l, struct r_scope_elem *scope[], const ElfW(Sym) **ref,
...
void _dl_relocate_object_no_relro(struct link_map *l, struct r_scope_elem *scope[],
...
void _dl_protect_relro(struct link_map *l)
{
    if (l->l_relro_size == 0) {
        return;
    }
...
void
__attribute_noinline__ _dl_reloc_bad_type(struct link_map *map, unsigned int type, int plt)
{
