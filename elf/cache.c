...
const char *glibc_hwcaps_subdirectory_name(const struct glibc_hwcaps_subdirectory *dir)
{
...
/* Helper for sorting struct glibc_hwcaps_subdirectory elements by
   name.  */
static int assign_glibc_hwcaps_indices_compare(const void *l, const void *r)
{
...
/* Count the number of hwcaps subdirectories which are actually
   used.  */
static size_t glibc_hwcaps_count(void)
{
    size_t count = 0;
    for (struct glibc_hwcaps_subdirectory *p = hwcaps; p != NULL; p = p->next)
        if (p->used) {
...
static void assign_glibc_hwcaps_indices(void)
{
...
static void print_entry(const char *lib, int flag, uint64_t hwcap,
...
static const char *glibc_hwcaps_string(struct cache_extension_all_loaded *ext,
...
/* Print an error and exit if the new-file cache is internally
   inconsistent.  */
static void check_new_cache(struct cache_file_new *cache)
{
...
static void print_extensions(struct cache_extension_all_loaded *ext)
{
...
static void write_extensions(int fd, uint32_t str_offset,
...
static inline uint64_t compute_hwcap_value(struct cache_entry *entry)
{
...
void add_to_cache(const char *path, const char *filename, const char *soname,
...
int search_aux_cache(struct stat *stat_buf, int *flags, unsigned int *isa_level,
                     char **soname)
{
    struct aux_cache_entry_id id;
...
static void insert_to_aux_cache(struct aux_cache_entry_id *id, int flags,
...
void add_to_aux_cache(struct stat *stat_buf, int flags, unsigned int isa_level,
                      const char *soname)
{
    struct aux_cache_entry_id id;
    id.ino = (uint64_t) stat_buf->st_ino;
