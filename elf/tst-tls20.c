...
static void access_mod(const char *modname, void *mod, int i)
{
...
static void do_test_no_depedency(void)
{
    for (int i = 0; i < NMOD; i++) {
        load_mod(i);
...
static int nmodules(uint32_t v)
{
    unsigned int r = 0;
    while (v >>= 1) {
        r++;
...
static inline bool is_mod_set(uint32_t g, uint32_t n)
{
    return (1U << (n - 1)) & g;
}

static void print_gap(uint32_t g)
{
    if (!test_verbose) {
        return;
    }
...
static void do_test_dependency(void)
{
...
static void do_test_invalid_dependency(bool bind_now)
{
