...
static void add_to_global_resize(struct link_map *new)
{
...
static void add_to_global_update(struct link_map *new)
{
    struct link_namespaces *ns = &GL(dl_ns)[new->l_ns];
...
static size_t scope_has_map(struct link_map *map, struct link_map *new)
{
    size_t cnt;
...
static size_t scope_size(struct link_map *map)
{
    size_t cnt;
...
static void resize_scopes(struct link_map *new)
{
...
static void update_scopes(struct link_map *new)
{
    for (unsigned int i = 0; i < new->l_searchlist.r_nlist; ++i) {
        struct link_map *imap = new->l_searchlist.r_list[i];
        int from_scope = 0;
...
/* Call _dl_add_to_slotinfo with DO_ADD set to false, to allocate
   ...
   TLS.  */
static bool resize_tls_slotinfo(struct link_map *new)
{
    bool any_tls = false;
    for (unsigned int i = 0; i < new->l_searchlist.r_nlist; ++i)
...
static void update_tls_slotinfo(struct link_map *new)
{
    for (unsigned int i = 0; i < new->l_searchlist.r_nlist; ++i) {
...
static void activate_nodelete(struct link_map *new)
{
...
static void _dl_open_relocate_one_object(struct dl_open_args *args, struct r_debug *r,
...
static void call_dl_init(void *closure)
{
...
static bool is_already_fully_open(struct link_map *map, int mode)
{
...
static void dl_open_worker_begin(void *a)
{
...
void *_dl_open(const char *file, int mode, const void *caller_dlopen, Lmid_t nsid,
...
void _dl_show_scope(struct link_map *l, int from)
{
