...
static char **get_next_env(char **envp, char **name, char **val, char ***prev_envp)
{
...
static bool tunable_parse_num(const char *strval, size_t len, tunable_num_t *val)
{
    char *endptr = NULL;
...
/* Validate range of the input value and initialize the tunable CUR if it looks
   good.  */
static bool tunable_initialize(tunable_t *cur, const char *strval, size_t len)
{
    tunable_val_t val = { 0 };
    ...
    if (cur->type.type_code != TUNABLE_TYPE_STRING) {
        ...
        val.strval = (struct tunable_str_t) {
        strval, len
    };
...
bool __tunable_is_initialized(tunable_id_t id)
{
...
static int parse_tunables_string(const char *valstring, struct tunable_toset_t *tunables)
{
    ...
    while (!done) {
        ...
        for (size_t i = 0; i < tunables_list_size; i++) {
            ...
            if (tunable_is_name(cur->name, name)) {
                tunables[i] = (struct tunable_toset_t) {
                    cur, value, p - value
                };
                break;
            }
...
static void parse_tunable_print_error(const struct tunable_toset_t *toset)
{
...
static void parse_tunables(const char *valstring)
{
...
void __tunable_get_default(tunable_id_t id, void *valp)
{
    tunable_t *cur = &tunable_list[id];
