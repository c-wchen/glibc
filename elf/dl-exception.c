...
/* Dummy allocation object used if allocating the message buffer
   fails.  */
static void oom_exception(struct dl_exception *exception)
{
    exception->objname = "";
...
/* Adjust the message buffer to indicate whether it is possible to
   ...
   pointer.  */
static void adjust_message_buffer(struct dl_exception *exception)
{
...
void _dl_exception_create(struct dl_exception *exception, const char *objname,
