...
static inline void rtld_timer_start(hp_timing_t *var)
{
    HP_TIMING_NOW(*var);
}

static inline void rtld_timer_stop(hp_timing_t *var, hp_timing_t start)
{
    hp_timing_t stop;
    HP_TIMING_NOW(stop);
...
static inline void rtld_timer_accum(hp_timing_t *sum, hp_timing_t start)
{
    hp_timing_t stop;
...
static void audit_list_init(struct audit_list *list)
{
    list->length = 0;
...
static void audit_list_add_string(struct audit_list *list, const char *string)
{
...
static void audit_list_add_dynamic_tag(struct audit_list *list, struct link_map *main_map,
                                       unsigned int tag)
{
...
static const char *audit_list_next(struct audit_list *list)
{
...
/* Count audit modules before they are loaded so GLRO(dl_naudit)
   is not yet usable.  */
static size_t audit_list_count(struct audit_list *list)
{
...
/* If we would use strong_alias here the compiler would see a
   ...
   visibility attribute.  */
extern struct rtld_global_ro _rtld_local_ro __attribute__((alias("_rtld_global_ro"), visibility("hidden")));
...
static void *init_tls(size_t naudit)
{
...
unsigned int handle_preload_list(const char *preloadlist, struct link_map *main_map,
                                 const char *where)
{
...
static void unload_audit_module(struct link_map *map, int original_tls_idx)
{
#ifndef NDEBUG
    Lmid_t ns = map->l_ns;
#endif
...
/* Called to print an error message if loading of an audit module
   failed.  */
static void report_audit_module_load_error(const char *name, const char *err_str,
        bool malloced)
{
    _dl_error_printf("\
...
static void load_audit_module(const char *name, struct audit_ifaces **last_audit)
{
...
static void load_audit_modules(struct link_map *main_map, struct audit_list *audit_list)
{
...
static void rtld_chain_load(struct link_map *main_map, char *argv0)
{
...
static bool rtld_setup_main_map(struct link_map *main_map)
{
...
/* Adjusts the contents of the stack and related globals for the user
   ...
   argv here.  */
static void _dl_start_args_adjust(int skip_args, int skip_env)
{
...
/* This is a little helper function for resolving symbols while
   tracing the binary.  */
static void print_unresolved(int errcode __attribute__((unused)), const char *objname,
...
/* Process the string given as the parameter which explains which debugging
   options are enabled.  */
static void process_dl_debug(struct dl_main_state *state, const char *dl_debug)
{
...

static int process_envvars_secure(struct dl_main_state *state)
{
    char **runp = _environ;
    char *envline;
    int skip_env = 0;
...
static void process_envvars_default(struct dl_main_state *state)
{
    char **runp = _environ;
    char *envline;
...
#if HP_TIMING_INLINE
static void print_statistics_item(const char *title, hp_timing_t time,
                                  hp_timing_t total)
{
