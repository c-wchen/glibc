...
/* Allocate a new subdirectory with full path PATH under ENTRY, using
   inode data from *ST.  */
static struct dir_entry *new_sub_entry(const struct dir_entry *entry, const char *path,
...
static bool add_single_dir(struct dir_entry *entry, int verbose)
{
...
static void add_glibc_hwcaps_subdirectories(struct dir_entry *entry, const char *path)
{
...
static void add_dir_1(const char *line, const char *from_file, int from_line)
{
    unsigned int i;
...
static int chroot_stat(const char *real_path, const char *path, struct stat *st)
{
    int ret;
    char *canon_path;
...
static void create_links(const char *real_path, const char *path, const char *libname,
                         const char *soname)
{
...
static bool skip_dso_based_on_name(const char *name, size_t len)
{
...
static void search_dir(const struct dir_entry *entry)
{
    ...
    while ((direntry = readdir64(dir)) != NULL) {
        ...
        } else if (__glibc_unlikely(lstat(real_file_name, &lstat_buf))) {
