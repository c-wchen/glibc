...
/* Print S of LEN bytes to standard output, quoting characters as
   needed.  */
static void print_string_length(const char *s, size_t len)
{
    _dl_putc('"');
...
void _dl_diagnostics_print_string(const char *s)
{
    if (s == NULL) {
        _dl_printf("0x0");
        return;
...
static bool unfiltered_envvar(const char *env, size_t *name_length)
{
