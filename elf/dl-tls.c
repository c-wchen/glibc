...
static inline void _dl_tls_allocate_begin(void)
{
    atomic_fetch_add_relaxed(&_dl_tls_threads_in_update, 1);
}

static inline void _dl_tls_allocate_end(void)
{
    atomic_fetch_add_relaxed(&_dl_tls_threads_in_update, -1);
}

static inline bool _dl_tls_allocate_active(void)
{
...
static inline int tls_static_surplus(int nns, int opt_tls)
{
...
void _dl_tls_static_surplus_init(size_t naudit)
{
    size_t nns, opt_tls;
...
void _dl_assign_tls_modid(struct link_map *l)
{
    size_t result;
...
static inline void **tcb_to_pointer_to_free_location(void *tcb)
{
#if TLS_TCB_AT_TP
...
static dtv_t *_dl_resize_dtv(dtv_t *dtv, size_t max_modid)
{
    /* Resize the dtv.  */
    dtv_t *newp;
...
/* Allocate initial TLS.  RESULT should be a non-NULL pointer to storage
   ...
   by dlopen.  */
void *_dl_allocate_tls_init(void *result, bool main_thread)
{
    if (result == NULL)
...
allocate_dtv_entry(size_t alignment, size_t size)
{
    ...
    return (struct dtv_pointer) {
        .val = aligned, .to_free = start
    };
}

static struct dtv_pointer allocate_and_init(struct link_map *map)
{
...
static inline void *tls_get_addr_adjust(void *from_dtv, tls_index *ti)
{
...
static void *
__attribute_noinline__ tls_get_addr_tail(tls_index *ti, dtv_t *dtv, struct link_map *the_map)
{
...
__attribute_noinline__ update_get_addr(tls_index *ti, size_t gen)
{
...
void _dl_tls_initial_modid_limit_setup(void)
{
    struct dtv_slotinfo_list *listp = GL(dl_tls_dtv_slotinfo_list);
    size_t idx;
...
bool _dl_add_to_slotinfo(struct link_map *l, bool do_add)
{
