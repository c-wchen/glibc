...
void __nisbind_destroy(dir_binding *bind)
{
    if (bind->clnt != NULL) {
        if (bind->use_auth) {
...
nis_error __nisbind_connect(dir_binding *dbp)
{
    nis_server *serv;
    u_short port;
...
static directory_obj *rec_dirsearch(const_nis_name name, directory_obj *dir, nis_error *status)
{
    fd_result *fd_res;
    XDR xdrs;
...
static directory_obj *first_shoot(const_nis_name name, directory_obj *dir)
{
...
static void nis_server_cache_add(const_nis_name name, int search_parent,
...
nis_error __nisfind_server(const_nis_name name, int search_parent,
