...
static const char *nis_nstype2str(const nstype type)
{
...
static const char *nis_objtype(const zotypes type)
{
    switch (type) {
        case NIS_BOGUS_OBJ:
...
static void print_ttl(const uint32_t ttl)
{
    uint32_t time, s, m, h;
...
static void nis_print_objtype(enum zotypes type)
{
    printf("%s\n", nis_objtype(type));
}

void nis_print_rights(const unsigned int access)
{
    char result[17];
    unsigned int acc;
    int i;
