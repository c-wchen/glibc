...
/* Returns true if *ENTRY matches last_entry, based on
   data->ut_type.  */
static bool matches_last_entry(const struct utmp *data)
{
    if (file_offset <= 0)
...
static bool try_file_lock(int fd, int type)
{
...
static bool maybe_setutent(void)
{
...
int __libc_getutent_r(struct utmp *buffer, struct utmp **result)
{
    int saved_errno = errno;
...
static int internal_getut_nolock(const struct utmp *id)
{
    while (1) {
        ssize_t nbytes = read_last_entry();
        if (nbytes < 0) {
...
/* Search for *ID, updating last_entry and file_offset.  Return 0 on
   ...
   true to *LOCK_FAILED.  */
static int internal_getut_r(const struct utmp *id, bool *lock_failed)
{
    if (try_file_lock(file_fd, F_RDLCK)) {
        *lock_failed = true;
        return -1;
...
int __libc_getutid_r(const struct utmp *id, struct utmp *buffer,
                     struct utmp **result)
{
    if (!maybe_setutent()) {
        *result = NULL;
...
/* For implementing this function we don't use the getutent_r function
   because we can avoid the reposition on every new entry this way.  */
int __libc_getutline_r(const struct utmp *line, struct utmp *buffer,
                       struct utmp **result)
{
    if (!maybe_setutent()) {
        *result = NULL;
...
int __libc_updwtmp(const char *file, const struct utmp *utmp)
{
    int result = -1;
    off64_t offset;
    int fd;
