...
static struct hol *make_hol(const struct argp *argp, struct hol_cluster *cluster)
{
    char *so;
...
static struct hol_cluster *hol_add_cluster(struct hol *hol, int group, const char *header, int index,
...
static inline int until_short(const struct argp_option *opt, const struct argp_option *real,
                              const char *domain, void *cookie)
{
...
/* Returns the entry in HOL with the long option name NAME, or NULL if there is
   none.  */
static struct hol_entry *hol_find_entry(struct hol *hol, const char *name)
{
...
static int group_cmp(int group1, int group2)
{
...
static int hol_sibling_cluster_cmp(const struct hol_cluster *cl1,
...
static int hol_cousin_cluster_cmp(const struct hol_cluster *cl1,
                                  const struct hol_cluster *cl2)
{
...
/* If the option REAL has an argument, we print it in using the printf
   ...
   optional argument.  */
static void arg(const struct argp_option *real, const char *req_fmt, const char *opt_fmt,
...
static const char *filter_doc(const char *doc, int key, const struct argp *argp,
                              const struct argp_state *state)
{
...
/* Prints STR as a header line, with the margin lines set appropriately, and
   ...
   to 0.  */
static void print_header(const char *str, const struct argp *argp,
...
static void hol_entry_help(struct hol_entry *entry, const struct argp_state *state,
...
static int argp_args_usage(const struct argp *argp, const struct argp_state *state,
...
/* Output a usage message for ARGP to STREAM.  If called from
   ...
   needed. */
static void _help(const struct argp *argp, const struct argp_state *state, FILE *stream,
