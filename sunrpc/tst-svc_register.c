...
/* Server callback for the unused RPC service which is registered and
   unregistered.  */
static void server_dispatch(struct svc_req *request, SVCXPRT *transport)
{
    FAIL_EXIT1("server_dispatch called");
}

/* The port on which rpcbind listens for incoming requests.  */
static inline struct sockaddr_in rpcbind_address(void)
{
...
static bool_t xdr_test_state(XDR *xdrs, void *data, ...)
{
...
static void rpcbind_dispatch(struct svc_req *request, SVCXPRT *transport)
{
...
static void run_rpcbind(int rpcbind_sock)
{
...
/* Call out to the rpcbind test server to retrieve the test status
   information.  */
static struct test_state get_test_state(void)
{
    int socket = RPC_ANYSOCK;
    struct sockaddr_in address = rpcbind_address();
    CLIENT *client = clntudp_create
    (&address, PMAPPROG, PMAPVERS, (struct timeval) {
        1, 0
    }, &socket);
...
/* RPC test server.  Used to verify the svc_unregister behavior during
   thread cleanup.  */
static void *test_server_thread(void *closure)
{
