...
static void close_range_test_max_upper_limit(void)
{
...
static void close_range_test_common(int lowfd, unsigned int flags)
{
...
static void close_range_test(void)
{
    struct support_descriptors *descrs = support_descriptors_list();
...
#ifdef __linux__
_Noreturn static int close_range_test_fn(void *arg)
{
...
static void close_range_test_subprocess(void)
{
    struct support_descriptors *descrs = support_descriptors_list();
...
_Noreturn static int close_range_unshare_test_fn(void *arg)
{
    int lowfd = (int)((uintptr_t) arg);
...
static void close_range_unshare_test(void)
{
...
static bool is_in_array(int *arr, size_t len, int fd)
{
    bool r = false;
    for (int i = 0; i < len; i++)
        if (arr[i] == fd) {
...
static void close_range_cloexec_test(void)
{
    /* Check if the temporary file descriptor has no no gaps.  */
    int lowfd = support_open_dev_null_range(NFDS, O_RDONLY, 0600);
