...
static void un(const char *file)
{
    struct stat st;
    /* Does the file exist?  */
    if (lstat(file, &st) < 0
...
static void debug_cb(const char *which, const char *fpath,
...
static void check_same_stats(const struct stat *sb, const char *as_fname)
{
    struct stat as;
...
static int callback_phys(const char *fpath, const struct stat *sb, int typeflags, struct FTW *ftwbuf)
{
...
static int callback_log(const char *fpath, const struct stat *sb, int typeflags, struct FTW *ftwbuf)
{
