...
/* Error-checking wrapper for the openat function, compatible with the
   wrapper_func type.  */
static int wrap_openat(const char *path, int flags, mode_t mode)
{
...
/* Error-checking wrapper for the openat64 function, compatible with the
   wrapper_func type.  */
static int wrap_openat64(const char *path, int flags, mode_t mode)
{
...
/* Return true if FD is flagged as deleted in /proc/self/fd, false if
   not.  */
static bool is_file_deleted(int fd)
{
...
static void check_wrapper_flags_mode(const char *op, wrapper_func wrapper,
...
/* Check OP/WRAPPER with various flags at a specific PATH and
   MODE.  */
static void check_wrapper_mode(const char *op, wrapper_func wrapper,
...
static void check_wrapper(const char *op, wrapper_func wrapper,
                          const char *path)
{
...
static bool probe_path(const char *path)
{
