...
static inline int ns_rr_cursor_rcode(const struct ns_rr_cursor *c)
{
    return c->begin[3] & 0x0f;    /* Lower 4 bits at offset 3.  */
}

/* Returns the length of the answer section according to the DNS header.  */
static inline int ns_rr_cursor_ancount(const struct ns_rr_cursor *c)
{
...
/* Returns the length of the authority (name server) section according
   to the DNS header.  */
static inline int ns_rr_cursor_nscount(const struct ns_rr_cursor *c)
{
...
/* Returns the length of the additional data section according to the
   DNS header.  */
static inline int ns_rr_cursor_adcount(const struct ns_rr_cursor *c)
{
...
/* Returns a pointer to the uncompressed question name in wire
   format.  */
static inline const unsigned char *ns_rr_cursor_qname(const struct ns_rr_cursor *c)
{
    return c->begin + 12;     /* QNAME starts right after the header.  */
}

/* Returns the question type of the first and only question.  */
static inline const int ns_rr_cursor_qtype(const struct ns_rr_cursor *c)
{
...
static inline const int ns_rr_cursor_qclass(const struct ns_rr_cursor *c)
{
