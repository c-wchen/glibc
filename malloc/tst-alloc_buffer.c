...
/* Check for success obtaining empty arrays.  Does not assume the
   buffer is empty.  */
static void test_empty_array(struct alloc_buffer refbuf)
{
...
static void test_impossible_array(struct alloc_buffer refbuf)
{
    if (test_verbose)
...
static void test_after_failure(struct alloc_buffer refbuf)
{
    if (test_verbose)
        printf("info: %s: current=0x%llx end=0x%llx\n",
               __func__, (unsigned long long) refbuf.__alloc_buffer_current,
...
static void test_empty(struct alloc_buffer refbuf)
{
...
static void test_size_1(struct alloc_buffer refbuf)
{
    TEST_VERIFY(!alloc_buffer_has_failed(&refbuf));
...
static void test_size_2(struct alloc_buffer refbuf)
{
    TEST_VERIFY(!alloc_buffer_has_failed(&refbuf));
...
static void test_misaligned(char pad)
{
    enum { SIZE = 23 };
...
static void test_large_misaligned(void)
{
    uintptr_t minus1 = -1;
