...
static void gettime(struct entry *e)
{
#if HP_TIMING_INLINE
    hp_timing_t now;
    HP_TIMING_NOW(now);
...
static inline void peak_atomic_max(_Atomic size_t *peak, size_t val)
{
    size_t v;
    do {
...
static void update_data(struct header *result, size_t len, size_t old_len)
{
    if (result != NULL) {
