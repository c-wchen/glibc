...
static void *fork_thread_function(void *closure)
{
...
static void *read_thread_function(void *closure)
{
...
static void *flushall_thread_function(void *closure)
{
    while (!__atomic_load_n(&termination_requested, __ATOMIC_RELAXED))
        if (fflush(NULL) != 0) {
...
static void create_threads(pthread_t *threads, size_t count, void *(*func)(void *))
{
...
static void join_threads(pthread_t *threads, size_t count)
{
    for (size_t i = 0; i < count; ++i) {
...
static void create_file_with_large_line(void)
{
