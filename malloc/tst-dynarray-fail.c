...
static struct heap_filler *fill_heap(void)
{
    size_t pad = 4096;
...
static void free_fill_heap(struct heap_filler *head)
{
    while (head != NULL) {
...
/* Check allocation failures for int arrays (without an element free
   function).  */
static void test_int_fail(void)
{
...
static void test_str_fail(void)
{
    /* Exercise failure in add/emplace.
...
static void xsetrlimit_as(const struct rlimit *limit)
{
...
/* Limit the size of the process, so that memory allocation in
   ...
   it.  */
static void reduce_rlimit_as(void)
{
    struct rlimit limit;
