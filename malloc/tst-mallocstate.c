...
static void *dumped_heap_alloc(size_t length)
{
...
static unsigned int rand_next(unsigned long long *seed)
{
...
/* Fill LENGTH bytes at BUFFER with random contents, as determined by
   SEED.  */
static void randomize_buffer(unsigned char *buffer, size_t length,
...
static void dump_hex(unsigned char *buffer, size_t length)
{
...
/* Check that the allocation task allocation has the expected
   contents.  */
static void check_allocation(const struct allocation *alloc, int index)
{
...
static void shuffle_allocation_tasks(void)
{
...
static void initial_allocations(void)
{
...
static void heap_activity(void)
{
...
static void heap_activity_deallocate(void)
{
...
static void full_heap_check(void)
{
    /* Dumped heap.  */
    for (int i = 0; i < allocation_task_count; ++i)
