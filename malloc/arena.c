...
static __always_inline size_t heap_min_size(void)
{
    return mp_.hp_pagesize == 0 || mp_.hp_pagesize > HEAP_MAX_SIZE
           ? HEAP_MIN_SIZE : mp_.hp_pagesize;
}

static __always_inline size_t heap_max_size(void)
{
...
static __always_inline heap_info *heap_for_ptr(void *ptr)
{
    size_t max_size = heap_max_size();
    return PTR_ALIGN_DOWN(ptr, max_size);
}

static __always_inline struct malloc_state *arena_for_chunk(mchunkptr ptr)
{
...
static heap_info *alloc_new_heap(size_t size, size_t top_pad, size_t pagesize,
                                 int mmap_flags)
{
    char *p1, *p2;
    unsigned long ul;
