...
ATTRIBUTE_PURE_114833 static ptrdiff_t sumsize(ptrdiff_t a, ptrdiff_t b)
{
#ifdef ckd_add
    ptrdiff_t sum;
...
static void *xmalloc(ptrdiff_t size)
{
    void *p = malloc(size);
    if (!p) {
...
static struct tm *gmtime_r(time_t *tp, struct tm *tmp)
{
...
static struct tm *localtime_rz(ATTRIBUTE_MAYBE_UNUSED timezone_t rz, time_t *tp, struct tm *tmp)
{
...
static time_t mktime_z(timezone_t tz, struct tm *tmp)
{
    return mktime(tmp);
}
# endif
...
static void tzfree(ATTRIBUTE_MAYBE_UNUSED timezone_t initial_environ)
{
# if !HAVE_SETENV
...
static struct tm *my_localtime_rz(timezone_t tz, time_t *tp, struct tm *tmp)
{
...
static char const *saveabbr(char **buf, ptrdiff_t *bufalloc, struct tm const *tmp)
{
...
static time_t hunt(timezone_t tz, time_t lot, time_t hit, bool only_ok)
{
    static char        *loab;
...
static intmax_t delta_nonneg(struct tm *newp, struct tm *oldp)
{
...
static long gmtoff(struct tm const *a, ATTRIBUTE_MAYBE_UNUSED time_t *t,
...
static void showextrema(timezone_t tz, char *zone, time_t lo, struct tm *lotmp, time_t hi)
{
...
static int my_snprintf(char *s, size_t size, char const *format, ...)
{
    int n;
    va_list args;
    char const *arg;
...
static int format_local_time(char *buf, ptrdiff_t size, struct tm const *tm)
{
...
static int format_utc_offset(char *buf, ptrdiff_t size, struct tm const *tm, time_t t)
{
...
static ptrdiff_t format_quoted_string(char *buf, ptrdiff_t size, char const *p)
{
    char *b = buf;
    ptrdiff_t s = size;
    if (!s) {
...
static bool istrftime(char *buf, ptrdiff_t size, char const *time_fmt,
...
static void showtrans(char const *time_fmt, struct tm const *tm, time_t t, char const *ab,
...
static char const *abbr(struct tm const *tmp)
{
#ifdef TM_ZONE
    return tmp->TM_ZONE;
#else
