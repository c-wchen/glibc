...
static long int mkbuf(char *buf, bool neg, bool colon, unsigned int hhmm, size_t ndigits)
{
    const int mm_max = 59;
    char sign = neg ? '-' : '+';
    int i;
...
static void describe(bool string_valid, long int tm_gmtoff)
{
    if (string_valid) {
...
static int compare(const char *buf, long int expect, unsigned int *nresult)
{
    struct tm tm;
    char *p;
