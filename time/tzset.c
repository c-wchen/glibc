...
static char *__tzstring_len(const char *s, size_t len)
{
    char *p;
...
static unsigned int compute_offset(unsigned int ss, unsigned int mm, unsigned int hh)
{
    if (ss > 59) {
        ss = 59;
    }
...
static bool parse_tzname(const char **tzp, int whichrule)
{
...
/* Parses the time zone offset at *TZP, and writes it to
   ...
   successful.  */
static bool parse_offset(const char **tzp, int whichrule)
{
    const char *tz = *tzp;
    if (whichrule == 0
...
static bool parse_rule(const char **tzp, int whichrule)
{
    const char *tz = *tzp;
...
void __tz_compute(__time64_t timer, struct tm *tm, int use_localtime)
{
...
void __libc_tzset_freemem(void)
{
