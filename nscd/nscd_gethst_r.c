...
int __nscd_gethostbyname_r(const char *name, struct hostent *resultbuf,
...
int __nscd_gethostbyname2_r(const char *name, int af, struct hostent *resultbuf,
                            char *buffer, size_t buflen, struct hostent **result,
                            int *h_errnop)
{
...
int __nscd_gethostbyaddr_r(const void *addr, socklen_t len, int type,
...
/* Note that we only free the structure if necessary.  The memory
   ...
   handling.  */
void __nscd_hst_map_freemem(void)
{
