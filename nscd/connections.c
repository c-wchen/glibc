...
static uint32_t __bump_nl_timestamp(void)
{
...
static int check_use(const char *data, nscd_ssize_t first_free, uint8_t *usemap,
...
void install_watches(struct traced_file *finfo)
{
...
void register_traced_file(size_t dbidx, struct traced_file *finfo)
{
...
#ifdef SCM_RIGHTS
static void send_ro_fd(struct database_dyn *db, char *key, int fd)
{
...
static char *read_cmdline(size_t *size)
{
...
static bool restart_p(time_t now)
{
...
int check_file(struct traced_file *finfo)
{
    struct stat64 st;
...
static void inotify_check_files(bool *to_clear, union __inev *inev)
{
...
/* If an entry in the array of booleans TO_CLEAR is TRUE then clear the cache
   ...
   have LASTDB entries.  */
static inline void clear_db_cache(bool *to_clear)
{
    for (size_t dbcnt = 0; dbcnt < lastdb; ++dbcnt)
        if (to_clear[dbcnt]) {
...
int handle_inotify_events(void)
{
...
#ifdef HAVE_EPOLL
static void main_loop_epoll(int efd)
{
