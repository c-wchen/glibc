...
int __nscd_getpwnam_r(const char *name, struct passwd *resultbuf, char *buffer,
                      size_t buflen, struct passwd **result)
{
    if (name == NULL) {
        return -1;
...
int __nscd_getpwuid_r(uid_t uid, struct passwd *resultbuf, char *buffer,
                      size_t buflen, struct passwd **result)
{
...
/* Note that we only free the structure if necessary.  The memory
   ...
   handling.  */
void __nscd_pw_map_freemem(void)
{
