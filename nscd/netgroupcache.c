...
/* Send a notfound response to FD.  Always returns -1 to indicate an
   ephemeral error.  */
static time_t send_notfound(int fd)
{
    if (fd != -1) {
...
/* Sends a notfound message and prepares a notfound dataset to write to the
   ...
   dataset. */
static bool do_notfound(struct database_dyn *db, int fd, request_header *req,
...
static void addgetnetgrentX_scratch_init(struct addgetnetgrentX_scratch *scratch)
{
...
static void addgetnetgrentX_scratch_free(struct addgetnetgrentX_scratch *scratch)
{
...
static char *addgetnetgrentX_append_n(struct addgetnetgrentX_scratch *scratch,
...
static bool addgetnetgrentX_append(struct addgetnetgrentX_scratch *scratch, const char *s)
{
    if (s == NULL) {
        s = "";
    }
...
static time_t addgetnetgrentX(struct database_dyn *db, int fd, request_header *req,
...
static time_t addinnetgrX(struct database_dyn *db, int fd, request_header *req,
...
static time_t addgetnetgrentX_ignore(struct database_dyn *db, int fd, request_header *req,
                                     const char *key, uid_t uid, struct hashentry *he,
                                     struct datahead *dh)
{
    struct addgetnetgrentX_scratch scratch;
...
void addgetnetgrent(struct database_dyn *db, int fd, request_header *req,
                    void *key, uid_t uid)
{
...
time_t readdgetnetgrent(struct database_dyn *db, struct hashentry *he,
                        struct datahead *dh)
{
    request_header req = {
        .type = GETNETGRENT,
...
void addinnetgr(struct database_dyn *db, int fd, request_header *req,
                void *key, uid_t uid)
{
...
time_t readdinnetgr(struct database_dyn *db, struct hashentry *he,
                    struct datahead *dh)
{
    request_header req = {
        .type = INNETGR,
