...
cap_t preserve_capabilities(void)
{
    if (getuid() != 0)
...
void install_real_capabilities(cap_t new_caps)
{
...
/* Determine if we are running on an SELinux kernel. Set selinux_enabled
   to the result.  */
void nscd_selinux_enabled(int *selinux_enabled)
{
...
DIAG_PUSH_NEEDS_COMMENT DIAG_IGNORE_NEEDS_COMMENT(10, "-Wdeprecated-declarations");

/* Initialize the user space access vector cache (AVC) for NSCD along with
   log/thread/lock callbacks.  */
void nscd_avc_init(void)
{
...
/* Check the permission from the caller (via getpeercon) to nscd.
   ...
   up.  */
int nscd_request_avc_has_perm(int fd, request_type req)
{
...
void nscd_avc_cache_stats(struct avc_cache_stats *cstats)
{
...
void nscd_avc_print_stats(struct avc_cache_stats *cstats)
{
