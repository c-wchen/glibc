...
static int detect_conflict(const char *alias)
{
...
static void add_alias2(const char *from, const char *to, const char *wp)
{
...
static void insert_module(struct gconv_module *newp, int tobefreed)
{
...
/* Add new module.  */
static void add_module(char *rp, const char *directory, size_t dir_len, int modcounter)
{
    /* We expect now
       1. `from' name
       2. `to' name
...
static void __gconv_get_path(void)
{
...
static void __gconv_read_conf(void)
{
...
void __gconv_conf_freemem(void)
{
