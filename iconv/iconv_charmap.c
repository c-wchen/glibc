...
static inline struct convtable *allocate_table(void)
{
    return (struct convtable *) xcalloc(1, sizeof(struct convtable));
}

static inline void free_table(struct convtable *tbl)
{
    free(tbl);
}


static inline int is_term(struct convtable *tbl, unsigned int idx)
{
    return tbl->term[idx / 8] & (1 << (idx % 8));
}


static inline void clear_term(struct convtable *tbl, unsigned int idx)
{
    tbl->term[idx / 8] &= ~(1 << (idx % 8));
}


static inline void set_term(struct convtable *tbl, unsigned int idx)
{
    tbl->term[idx / 8] |= 1 << (idx % 8);
}
...
int charmap_conversion(const char *from_code, struct charmap_t *from_charmap,
...
static void add_bytes(struct convtable *tbl, const struct charseq *in, struct charseq *out)
{
    int n = 0;
    unsigned int byte;
...
static struct charseq *convert_charseq(iconv_t cd, const struct charseq *seq)
{
...
static int process_block(struct convtable *tbl, char *addr, size_t len, FILE *output)
{
    size_t n = 0;
