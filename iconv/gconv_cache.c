...
static int find_module_idx(const char *str, size_t *idxp)
{
    unsigned int idx;
    unsigned int hval;
    unsigned int hval2;
    ...
    while (hashtab[idx].string_offset != 0)
        if (hashtab[idx].string_offset < limit
            ...
            *idxp = hashtab[idx].module_idx;
            return 0;
        } else if ((idx += hval2) >= header->hash_size) {
...
#ifndef STATIC_GCONV
static int find_module(const char *directory, const char *filename,
...
int __gconv_compare_alias_cache(const char *name1, const char *name2, int *result)
{
    size_t name1_idx;
    size_t name2_idx;
...
int __gconv_lookup_cache(const char *toset, const char *fromset,
...
void __gconv_release_cache(struct __gconv_step *steps, size_t nsteps)
{
    if (gconv_cache != NULL)
...
void __gconv_cache_freemem(void)
{
    if (cache_malloced) {
        free(gconv_cache);
    }
