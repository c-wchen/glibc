...
/* Write out the range [first, last), terminating the process on write
   error.  */
static void write_fully(int fd, const char *first, const char *last)
{
    while (first < last) {
...
/* CD is the iconv handle.  Input processing starts at *ADDR, and
   ...
   sequence.  */
static int process_block(iconv_t cd, char **addr, size_t *len, off64_t file_offset,
                         bool *incomplete)
{
...
static void add_known_names(struct gconv_module *node)
{
    if (node->left != NULL) {
