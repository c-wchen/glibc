...
static void do_filler_shared(void)
{
...
static inline void critical_section(int length)
{
...
static inline void non_critical_section(int length)
{
    for (int i = length; i >= 0; i--) {
        UNIT_WORK_NON_CRT;
    }
...
static double do_one_test(int num_threads, int crt_len, int non_crt_len, long iters)
{
    int i;
    timing_t mean;
...
static void do_bench_one(const char *name, int num_threads, int crt_len, int non_crt_len,
                         json_ctx_t *js)
{
    timing_t cur;
