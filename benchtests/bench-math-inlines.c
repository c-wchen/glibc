...
static __always_inline int __isnan_inl(double d)
{
    uint64_t di;
...
static __always_inline int __isinf_ns2(double d)
{
    uint64_t di;
    EXTRACT_WORDS64(di, d);
    return (di & 0x7fffffffffffffffull) == 0x7ff0000000000000ull;
...
static __always_inline int __finite_inl(double d)
{
    uint64_t di;
    EXTRACT_WORDS64(di, d);
    return (di & 0x7fffffffffffffffull) < 0x7ff0000000000000ull;
...
static __always_inline double remainder_test1(double x)
{
    double y = rem1;
...
static __always_inline double remainder_test2(double x)
{
    double y = rem1;
...
static void do_one_test(json_ctx_t *json_ctx, proto_t test_fn, volatile double *arr,
